/* 我的规则仓库·测试版 Bootstrap v1.0.2 */
var RULE_REPO_BOOTSTRAP_VERSION='1.0.2-test';
...
var RULE_REPO_CONFIG={
  id:'rule-repo-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',latestPath:'apps/tools/rule-repo/test.json',minBuild:353,
  moduleHeaders:{'Cache-Control':'no-cache'},
  defaultRelease:{schema:1,id:'rule-repo-test',name:'我的规则仓库·测试版',version:'3.5.0-rc4',build:353,ref:'main',modules:[
    ...
    {name:'uiPatch',path:'apps/tools/rule-repo/releases/3.5.0-rc4/ui_patch.js'},
    {name:'home',path:'apps/tools/rule-repo/releases/3.5.0-rc4/home.js'},
    ...
    {name:'updates',path:'apps/tools/rule-repo/releases/3.5.0-rc4/updates.js'},
    {name:'detail',path:'apps/tools/rule-repo/releases/3.5.0-rc4/detail.js'},
    {name:'history',path:'apps/tools/rule-repo/releases/3.4.0/history.js'},
    {name:'settings',path:'apps/tools/rule-repo/releases/3.5.0-rc4/settings.js'},
    {name:'candidatePatch',path:'apps/tools/rule-repo/releases/3.5.0-rc4/candidate_patch.js'}
  ],verify:{global:'HikerRuleRepo',property:'version',equals:'3.5.0-rc4'}}
};
var RuleRepoBoot={
  ...
  updatePage:function(){setPageTitle('测试版更新');var d=[],m=this.manager(),info=m.info(RULE_REPO_CONFIG),cur=info.current||RULE_REPO_CONFIG.defaultRelease,prev=info.previous||null;d.push({title:'我的规则仓库·测试版',desc:'当前 '+cur.version+' / build '+cur.build+'\nBootstrap '+RULE_REPO_BOOTSTRAP_VERSION+' · Manager '+info.managerVersion+(prev?'\n可回退 '+prev.version:''),col_type:'text_1',url:'hiker://empty'});d.push({title:'检查测试版更新',desc:'读取云端 test.json，测试通道独立于正式版。',col_type:'text_1',url:$('#noLoading#').lazyRule(function(){showLoading('检查更新…');try{require(RULE_REPO_MANAGER_URL,{headers:{'Cache-Control':'no-cache'}},201);var x=HikerCloudRemote.check(RULE_REPO_CONFIG);hideLoading();return x.hasUpdate?'toast://发现测试版 '+x.latest.version:'toast://已是当前测试版 '+x.current.version;}catch(e){hideLoading();return'toast://检查失败：'+String(e.message||e);}})});d.push({title:'升级测试版',desc:'预加载并验证新测试 release，成功后才切换当前版本。',col_type:'text_1',url:$('#noLoading#').lazyRule(function(){showLoading('升级测试版…');try{require(RULE_REPO_MANAGER_URL,{headers:{'Cache-Control':'no-cache'}},201);var r=HikerCloudRemote.update(RULE_REPO_CONFIG);hideLoading();if(!r.ok)return'toast://升级失败：'+r.error;if(!r.changed)return'toast://已是当前测试版';refreshPage(false);return'toast://已升级到 '+r.current.version;}catch(e){hideLoading();return'toast://升级失败：'+String(e.message||e);}})});d.push({title:'回退测试版',desc:'只回退测试版，不影响正式版。',col_type:'text_1',url:$('#noLoading#').lazyRule(function(){showLoading('回退测试版…');try{require(RULE_REPO_MANAGER_URL,{headers:{'Cache-Control':'no-cache'}},201);var r=HikerCloudRemote.rollback(RULE_REPO_CONFIG);hideLoading();if(!r.ok)return'toast://回退失败：'+r.error;refreshPage(false);return'toast://已回退到 '+r.current.version;}catch(e){hideLoading();return'toast://回退失败：'+String(e.message||e);}})});d.push({title:'重新加载当前测试版',desc:'清理当前测试业务模块缓存并重新下载。',col_type:'text_1',url:$('#noLoading#').lazyRule(function(){showLoading('重新加载…');try{require(RULE_REPO_MANAGER_URL,{headers:{'Cache-Control':'no-cache'}},201);var r=HikerCloudRemote.reinstall(RULE_REPO_CONFIG);hideLoading();if(!r.ok)return'toast://失败：'+r.error;refreshPage(false);return'toast://当前测试版已重新加载';}catch(e){hideLoading();return'toast://失败：'+String(e.message||e);}})});setResult(d);}
};
