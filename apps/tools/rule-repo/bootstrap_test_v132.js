/* 我的规则仓库·测试版 Bootstrap v1.0.32 - API-first mutable metadata */
var RULE_REPO_BOOTSTRAP_VERSION='1.0.32-test';
var RULE_REPO_MANAGER_URL='https://cdn.jsdelivr.net/gh/huoguotiankong/asset-core-7f3@main/libs/updater/v2.0.3/remote_manager.js';
...
var RULE_REPO_CONFIG={
 id:'rule-repo-test',branch:'main',latestPath:'apps/tools/rule-repo/test.json',minBuild:388,
 ...
 defaultRelease:{schema:1,id:'rule-repo-test',name:'我的规则仓库·测试版',version:'3.5.5-rc2',build:388,ref:'main',modules:[
  ...
  {name:'syncRefreshPatch',path:'apps/tools/rule-repo/releases/test-3.5.4-sync2/sync_refresh_patch.js'},
  {name:'iconDeliveryPatch',path:'apps/tools/rule-repo/releases/test-3.5.5-rc1/icon_delivery_patch.js'},
  {name:'channelIconPatch',path:'apps/tools/rule-repo/releases/test-3.5.5-rc2/channel_icon_patch.js'}
 ],verify:{global:'HikerRuleRepo',property:'version',equals:'3.5.5-rc2'}}
};
var RuleRepoBoot={
 manager:function(){require(RULE_REPO_MANAGER_URL+'?v=203',{headers:{'Cache-Control':'no-cache'}},203);if(typeof HikerCloudRemote!=='object')throw new Error('远程模块管理器加载失败');return HikerCloudRemote;},
 ...
 updatePage:function(){setPageTitle('测试版更新');var d=[],m=this.manager(),info=m.info(RULE_REPO_CONFIG),cur=info.current||RULE_REPO_CONFIG.defaultRelease,prev=info.previous||null,cfg=JSON.stringify(RULE_REPO_CONFIG),mgr=RULE_REPO_MANAGER_URL+'?v=203';
  d.push({title:'我的规则仓库·测试版',desc:'当前 '+cur.version+' · Build '+cur.build+'\n更新元数据：GitHub API 优先 · 模块：多镜像回退',img:RULE_REPO_ICON,pic_url:RULE_REPO_ICON,col_type:'icon_1_left_pic',url:'hiker://empty',extra:{lineVisible:false}});
  d.push({title:'检查更新',col_type:'text_2',url:$('#noLoading#').lazyRule(function(c,u){showLoading('正在检查…');try{var cfg=JSON.parse(c);require(u,{headers:{'Cache-Control':'no-cache'}},203);var x=HikerCloudRemote.check(cfg);hideLoading();return x.hasUpdate?'toast://发现 '+x.latest.version+' / Build '+x.latest.build:'toast://已经是最新测试版';}catch(e){hideLoading();return'toast://检查失败：'+String(e.message||e);}},cfg,mgr)});
  d.push({title:'升级测试版',col_type:'text_2',url:$('#noLoading#').lazyRule(function(c,u){showLoading('正在升级…');try{var cfg=JSON.parse(c);require(u,{headers:{'Cache-Control':'no-cache'}},203);var r=HikerCloudRemote.update(cfg);hideLoading();if(!r.ok)return'toast://升级失败：'+r.error;if(!r.changed)return'toast://已经是最新测试版';refreshPage(false);return'toast://已升级到 '+r.current.version;}catch(e){hideLoading();return'toast://升级失败：'+String(e.message||e);}},cfg,mgr)});
  d.push({col_type:'line'});
  d.push({title:'回退到上一版本',desc:prev?'可回退到 '+prev.version:'当前没有可回退版本',col_type:'text_1',url:$('#noLoading#').lazyRule(function(c,u){showLoading('正在回退…');try{var cfg=JSON.parse(c);require(u,{headers:{'Cache-Control':'no-cache'}},203);var r=HikerCloudRemote.rollback(cfg);hideLoading();if(!r.ok)return'toast://回退失败：'+r.error;refreshPage(false);return'toast://已回退到 '+r.current.version;}catch(e){hideLoading();return'toast://回退失败：'+String(e.message||e);}},cfg,mgr)});
  d.push({title:'重新加载当前版本',desc:'页面异常或模块缓存异常时使用',col_type:'text_1',url:$('#noLoading#').lazyRule(function(c,u){showLoading('正在重新加载…');try{var cfg=JSON.parse(c);require(u,{headers:{'Cache-Control':'no-cache'}},203);var r=HikerCloudRemote.reinstall(cfg);hideLoading();if(!r.ok)return'toast://失败：'+r.error;refreshPage(false);return'toast://已重新加载';}catch(e){hideLoading();return'toast://操作失败：'+String(e.message||e);}},cfg,mgr)});setResult(d);
 }
};
