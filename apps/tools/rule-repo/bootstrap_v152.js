/* 我的规则仓库 Remote Bootstrap v1.5.2 - Stable 3.5.2 */
var RULE_REPO_BOOTSTRAP_VERSION='1.5.2';
...
var RULE_REPO_CONFIG={
 id:'rule-repo',branch:'main',latestPath:'apps/tools/rule-repo/latest.json',minBuild:364,
 repoTemplates:[
 ...
 defaultRelease:{schema:1,id:'rule-repo',name:'我的规则仓库',version:'3.5.2',build:364,ref:'main',modules:[
  ...
  {name:'home',path:'apps/tools/rule-repo/releases/3.5.0-rc8/home.js'},
  ...
  {name:'channelPage',path:'apps/tools/rule-repo/releases/3.5.0-rc7/channel_page.js'},
  {name:'history',path:'apps/tools/rule-repo/releases/3.5.0-rc5/history.js'},
  {name:'settings',path:'apps/tools/rule-repo/releases/3.5.0-rc8/settings.js'},
  {name:'runtimeContract',path:'apps/tools/rule-repo/releases/3.5.0-rc9/runtime_contract.js'},
  {name:'stablePatch',path:'apps/tools/rule-repo/releases/3.5.2/stable_patch.js'},
  {name:'channelLocalPatch',path:'apps/tools/rule-repo/releases/3.5.2/channel_local_patch.js'}
 ],verify:{global:'HikerRuleRepo',property:'version',equals:'3.5.2'}}
};
var RuleRepoBoot={
 ...
 updatePage:function(){setPageTitle('正式版更新');var d=[],m=this.manager(),info=m.info(RULE_REPO_CONFIG),cur=info.current||RULE_REPO_CONFIG.defaultRelease,prev=info.previous||null,cfg=JSON.stringify(RULE_REPO_CONFIG),mgr=RULE_REPO_MANAGER_URL;
  d.push({title:'我的规则仓库',desc:'当前 '+cur.version+'\n稳定通道 · 可从仓库重新导入测试版',img:RULE_REPO_ICON,pic_url:RULE_REPO_ICON,col_type:'icon_1_left_pic',url:'hiker://empty',extra:{lineVisible:false}});
  ...
  d.push({title:'升级正式版',col_type:'text_2',url:$('#noLoading#').lazyRule(function(c,u){showLoading('正在升级…');try{var cfg=JSON.parse(c);require(u,{headers:{'Cache-Control':'no-cache'}},202);var r=HikerCloudRemote.update(cfg);hideLoading();if(!r.ok)return'toast://升级失败：'+r.error;if(!r.changed)return'toast://已经是最新正式版';refreshPage(false);return'toast://已升级到 '+r.current.version;}catch(e){hideLoading();return'toast://升级失败：'+String(e.message||e);}},cfg,mgr)});
  ...
  d.push({title:'重新加载当前版本',desc:'仅在页面异常或云端修复后需要使用',col_type:'text_1',url:$('#noLoading#').lazyRule(function(c,u){showLoading('正在重新加载…');try{var cfg=JSON.parse(c);require(u,{headers:{'Cache-Control':'no-cache'}},202);var r=HikerCloudRemote.reinstall(cfg);hideLoading();if(!r.ok)return'toast://失败：'+r.error;refreshPage(false);return'toast://已重新加载';}catch(e){hideLoading();return'toast://操作失败：'+String(e.message||e);}},cfg,mgr)});
  setResult(d);
 }
};
