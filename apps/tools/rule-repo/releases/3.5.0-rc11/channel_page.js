...
R.channelPage=function(parent){
 ...
 d.push(this.hero(parent.name,'选择适合你的版本\n正式版用于日常，测试版用于体验新功能',this.iconOf(parent),'hiker://empty'));
 d.push(this.compactInfo('当前运行',this.isTestChannel()?'测试版 '+this.version:'正式版 '+this.version,'hiker://empty'));
 if(stable){
  this.pushSpacer(d);this.pushSection(d,'正式版 · 推荐','稳定、安全，适合日常使用');
  d.push(this.channelCard(parent,stable,'正式版 · 稳定推荐',!this.isTestChannel()));
  d.push(this.channelImportAction(parent,stable,'导入正式版'));
  if(stable.highlights&&stable.highlights.length)d.push(this.compactInfo('本版特点',stable.highlights.slice(0,3).join(' · '),'hiker://empty'));
 }
 if(test){
  this.pushSpacer(d);this.pushSection(d,'测试版 · 抢先体验','用于验证新界面和新功能');
  d.push(this.channelCard(parent,test,'测试版 · 抢先体验',this.isTestChannel()));
  d.push(this.channelImportAction(parent,test,'导入测试版'));
  if(test.highlights&&test.highlights.length)d.push(this.compactInfo('本轮重点',test.highlights.slice(0,3).join(' · '),'hiker://empty'));
 }
 this.pushSpacer(d);d.push(this.compactInfo('恢复保障',this.isTestChannel()?'测试版异常时，打开正式版 → 我的规则仓库 → 导入测试版即可恢复。':'测试版异常时，在这里重新导入测试版；正式版会始终保留。','hiker://empty'));
 setResult(d);
};
})(HikerRuleRepo);
