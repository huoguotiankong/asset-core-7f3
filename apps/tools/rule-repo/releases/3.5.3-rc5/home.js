/* 我的规则仓库 3.5.3-rc5 - Target UI 3.0 home */
(function(R){
R.home=function(){
 ...
 d.push(this.hero('我的规则仓库','海阔视界专属 · 规则管理中心\nTarget UI 3.0 · 3.5.3-rc5',this.iconOf(repo),'hiker://page/ruleRepoAbout?rule=&simple=true'));
 d.push({title:'测试版 RC5',col_type:'scroll_button',url:'hiker://page/ruleRepoUpdate?rule=&simple=true',extra:{lineVisible:false}});d.push({title:'远程模块',col_type:'scroll_button',url:'hiker://page/ruleRepoUpdates?rule=&simple=true',extra:{lineVisible:false}});d.push({title:'可回退',col_type:'scroll_button',url:'hiker://page/ruleRepoUpdate?rule=&simple=true',extra:{lineVisible:false}});
 ...
 d.push({title:sort==='default'?'默认排序':(sort==='updated'?'最近更新':(sort==='name'?'名称排序':'版本排序')),url:$('默认排序,最近更新,名称,版本','排序方式').select(function(){var map={'默认排序':'default','最近更新':'updated','名称':'name','版本':'version'};putMyVar('hc_repo_home_sort',map[String(input||'默认排序')]||'default');refreshPage(false);return'hiker://empty';}),col_type:'scroll_button',extra:{lineVisible:false}});
 d.push({title:'筛选',url:'hiker://page/ruleRepoCategory?rule=&simple=true',col_type:'scroll_button',extra:{lineVisible:false}});
 d.push({title:'同步',url:$('#noLoading#').lazyRule(function(){showLoading('正在同步…');try{var r=$.require('hiker://page/ruleRepoCore'),x=r.syncManifest();r.clearPresenceCache&&r.clearPresenceCache();hideLoading();if(!x.ok)return'toast://同步失败，已保留当前目录';refreshPage(false);return x.fresh?'toast://云端目录已更新':'toast://当前已是最新目录';}catch(e){hideLoading();return'toast://同步失败，已保留当前目录';}}),col_type:'scroll_button',extra:{lineVisible:false}});
 d.push({title:'只看收藏',url:$('#noLoading#').lazyRule(function(){putMyVar('hc_repo_home_view','favorites');refreshPage(false);return'hiker://empty';}),col_type:'scroll_button',extra:{lineVisible:false}});
