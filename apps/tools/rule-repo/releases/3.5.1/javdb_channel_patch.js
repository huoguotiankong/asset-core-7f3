/* Rule Repo 3.5.1 build 362 - JavDB Stable/Test/Local channel support */
(function(R){
...
R.auditJavdbLocal=function(text){
  text=String(text||'');
  var bad=[
    'github.com','raw.githubusercontent.com','api.github.com','huoguotiankong',
    '@users.noreply.github.com','/mnt/data','/home/','/Users/','Hankun8'
  ],i;
...
R.buildJavdbLocal=function(x){
  ...
  var core=JDBCLOUD.get('core',7,'hc_build_jdb_core_3941_b362');
...
R._javdbBaseImportRule=R.importRule;
R.importRule=function(raw){
  ...
  }catch(e){return 'toast://JavDB导入失败：'+String(e.message||e);}
  return this._javdbBaseImportRule(raw);
};
R._javdbBasePushChannelCard=R.pushChannelCard;
R.pushChannelCard=function(d,parent,c,recommended){
  if(String(parent&&parent.id||'')!=='javdb-v3')return this._javdbBasePushChannelCard(d,parent,c,recommended);
  var self=this,ch=String(c.channel||''),label=ch==='stable'?'正式版':(ch==='test'?'测试版':'本地版'),badge=ch==='stable'?'远程稳定':(ch==='test'?'远程测试':'纯本地'),raw={
    ...
    tags:['版本',ch==='stable'?'正式':(ch==='test'?'测试':'本地')],mode:String(c.mode||'remote'),updatedAt:String(c.updatedAt||''),
    icon:String(c.icon||self.iconOf(parent)),openTitle:String(c.openTitle||c.name||parent.name||'')
  },item=self.normalizeItem(raw,0),status=self.displayStatus(item),hl=Array.isArray(c.highlights)?c.highlights.slice(0,3):[];
...
R._javdbBaseChannelPage=R.channelPage;
R.channelPage=function(parent){
  if(String(parent&&parent.id||'')!=='javdb-v3')return this._javdbBaseChannelPage(parent);
  ...
  d.push(this.hero(parent.name,'正式/测试同名覆盖 · 本地版独立并存',this.iconOf(parent),'hiker://empty'));
  if(stable)d.push({title:'Stable  '+String(stable.version||''),url:'hiker://empty',col_type:'text_3',extra:{lineVisible:false}});
  if(test)d.push({title:'Test  '+String(test.version||''),url:'hiker://empty',col_type:'text_3',extra:{lineVisible:false}});
  if(local)d.push({title:'Local  '+String(local.version||''),url:'hiker://empty',col_type:'text_3',extra:{lineVisible:false}});
  this.pushSpacer(d);this.pushSection(d,'正式版','远程代码 · 稳定日常使用');if(stable)this.pushChannelCard(d,parent,stable,true);
  this.pushSpacer(d);this.pushSection(d,'测试版','远程代码 · 新功能与修复先行验证');if(test)this.pushChannelCard(d,parent,test,false);
  this.pushSpacer(d);this.pushSection(d,'本地版','纯本地完整代码 · 不依赖私人 GitHub 运行');if(local)this.pushChannelCard(d,parent,local,false);
  this.pushSpacer(d);
  d.push({title:'三版本关系',desc:'正式版与测试版规则名都为“JavDB v3”，导入时互相覆盖；本地版规则名为“JavDB v3 本地版”，可以独立保留。',url:'hiker://empty',col_type:'text_1',extra:{lineVisible:false}});
  setResult(d);
};
R.build=362;
R.releaseLabel='Stable 3.5.1 · Build 362';
})(HikerRuleRepo);
