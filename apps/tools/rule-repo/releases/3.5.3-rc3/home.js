/* 我的规则仓库 3.5.3-rc3 - target-match home */
(function(R){
R.home=function(){
 ...
 this.pushSpacer(d);
 d.push(this.metricCard('全部程序',stats.all,'all',view,'hc_repo_home_view'));
 ...
 this.pushSpacer(d);this.pushSection(d,'我的程序',cat==='all'?'全部程序 · '+(this.isTestChannel()?'测试版':'正式版'):this.categoryName(cat));
 ...
 d.push({title:'筛选',url:'hiker://page/ruleRepoCategory?rule=&simple=true',col_type:'flex_button',extra:{lineVisible:false}});
 ...
 if(stats.updates>0)d.push(this.compactInfo('有 '+stats.updates+' 个程序可以更新','进入更新页统一处理','hiker://page/ruleRepoUpdates?rule=&simple=true'));
 this.pushSpacer(d);this.pushNav(d,'home');
 setResult(d);
};
})(HikerRuleRepo);
