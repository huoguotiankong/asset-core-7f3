/* 我的规则仓库 3.5.3-rc3 - Target Match UI system */
(function(R){
...
R.metricCard=function(title,value,view,active,key){
 var on=String(active)===String(view),txt=(on?'● ':'')+String(title)+'\n'+String(value);
 return{title:txt,col_type:'text_4',url:$('#noLoading#').lazyRule(function(k,v){putMyVar(k,v);refreshPage(false);return'hiker://empty';},key,view),extra:{lineVisible:false,textAlign:'center'}};
};
R.metricInfo=function(title,value){return{title:String(title)+'\n'+String(value),col_type:'text_4',url:'hiker://empty',extra:{lineVisible:false,textAlign:'center'}};};
R.scopeChip=function(title,value,active,key){
 var on=String(active)===String(value),safe=String(title||'').replace(/</g,'&lt;').replace(/>/g,'&gt;'),html=on?'<font color="#1677ff">● '+safe+'</font>':safe;
 return{title:html,col_type:'scroll_button',url:$('#noLoading#').lazyRule(function(k,v){putMyVar(k,v);refreshPage(false);return'hiker://empty';},key,value),extra:{lineVisible:false}};
};
R.categoryTile=function(title,count,id,active,key){
 var on=String(active)===String(id),safe=String(title||'').replace(/</g,'&lt;').replace(/>/g,'&gt;'),html=on?'<font color="#1677ff">● '+safe+' '+String(count||0)+'</font>':safe+' '+String(count||0);
 return{title:html,col_type:'text_3',url:$('#noLoading#').lazyRule(function(k,v){putMyVar(k,v);if(k==='hc_repo_category'){clearMyVar('hc_repo_sub');clearMyVar('hc_repo_tag');}refreshPage(false);return'hiker://empty';},key,id),extra:{lineVisible:false}};
};
R.quickAction=function(title,icon,url){return{title:title,img:this.uiIcon(icon),pic_url:this.uiIcon(icon),url:url||'hiker://empty',col_type:'icon_small_4',extra:{lineVisible:false}};};
...
R.statusLabel=function(item){var s=this.premiumStatusText(item);return s==='可更新'?'↑ 可更新':(s==='已记录'?'✓ 已记录':s);};
R.itemCard=function(item){
 ...
 else{meta=this.cleanVersion(item.version)+' · '+String(item.categoryName||'程序')+' · '+this.statusLabel(item);tags=(item.tags||[]).slice(0,3);desc=tags.length?tags.join(' / '):String(item.subCategory||item.desc||'');if(last)desc+=' · '+this.formatShortTime(last)+' 使用';}
...
R.categorySplitCell=function(title,value,active,key,isMain){
 var on=String(active)===String(value),prefix=on?'● ':'',extra={lineVisible:false,textAlign:'left'};
 return{title:prefix+String(title||''),col_type:'text_2',url:$('#noLoading#').lazyRule(function(k,v,main){putMyVar(k,v);if(main){clearMyVar('hc_repo_sub');clearMyVar('hc_repo_tag');}refreshPage(false);return'hiker://empty';},key,value,!!isMain),extra:extra};
};
...
R.ruleRepoChannelFallback=function(){
 ...
 return{schema:4,id:'rule-repo',name:'我的规则仓库',updatedAt:'2026-08-21',channels:[
  ...
  {channel:'test',label:'测试版',id:'rule-repo-test',name:'我的规则仓库·测试版',version:'3.5.3-rc3',baseVersion:'3.5.2',targetVersion:'3.5.3',build:368,displayVersion:'Test 3.5.3-rc3 · Build 368 · Shell 1.0.15',path:'apps/tools/rule-repo/rule_repo_test_v115.txt',mode:'remote',updatedAt:'2026-08-21',recommended:false,desc:'从 Stable 3.5.2 干净续线 · Target Match UI 第三轮',highlights:['首页结构贴近目标稿','分类双栏原生近似','统计卡与设置密度重构'],icon:icon}
 ]};
};
