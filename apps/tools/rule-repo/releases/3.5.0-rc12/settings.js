/* 我的规则仓库 v3.5.0-rc12 - product settings */
(function(R){
R.settingsPage=function(){
 ...
 this.pushSection(d,'同步与更新','保持自动、稳定，异常时优先保留可用数据');
 ...
 this.pushSpacer(d);this.pushSection(d,'关于','低频信息统一后置');
...
R.aboutPage=function(){
 ...
 this.pushSection(d,'核心能力','');d.push(this.quickAction('分类管理','category','hiker://page/ruleRepoCategory?rule=&simple=true'));d.push(this.quickAction('搜索','search','hiker://page/ruleRepoSearch?rule=&simple=true'));d.push(this.quickAction('版本更新','updates','hiker://page/ruleRepoUpdates?rule=&simple=true'));d.push(this.quickAction('备份恢复','history','hiker://page/ruleRepoSettings?rule=&simple=true'));
 this.pushSpacer(d);d.push(this.compactInfo('当前版本',this.version+' · '+(this.isTestChannel()?'测试通道':'正式通道'),'hiker://page/ruleRepoUpdate?rule=&simple=true'));d.push(this.compactInfo('产品原则','一个程序一个入口 · 正式/测试分层 · 常用操作优先 · 异常可恢复','hiker://empty'));
 this.pushSpacer(d);this.pushSection(d,'本版说明','当前测试版的重点变化与验证项');var log='';try{log=this.apiText('docs/RELEASE_RULE_REPO_RC12.md');log=String(log||'').slice(0,5000);}catch(e){log='本版说明暂时无法读取。';}d.push({title:log,url:'hiker://empty',col_type:'long_text',extra:{lineVisible:false,textSize:14}});setResult(d);
};
})(HikerRuleRepo);
