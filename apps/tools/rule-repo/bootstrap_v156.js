/* 我的规则仓库 Remote Bootstrap v1.5.6 - Stable 3.5.6 / Verified Install Index 14.2 */
(function(){
...
var BASE_SHA='5036c15d8beabd4ade6482b0bcdd02910ceb6d43',BASE_PATH='apps/tools/rule-repo/bootstrap_v155.js';
reqAny([
...
],156,'Stable Bootstrap 3.5.5 基线');
RULE_REPO_BOOTSTRAP_VERSION='1.5.6';
...
RULE_REPO_CONFIG.minBuild=393;
var d=JSON.parse(JSON.stringify(RULE_REPO_CONFIG.defaultRelease)),mods=[],src=d.modules||[];
for(var i=0;i<src.length;i++){var n=String(src[i]&&src[i].name||'');if(n==='fastHomeUpdateIndexV2'||n==='verifiedDeviceInstallIndex'||n==='stablePromotionPatch356')continue;mods.push(src[i]);}
mods.push({name:'fastHomeUpdateIndexV2',path:'apps/tools/rule-repo/releases/test-3.5.6-rc2/home_fast_update_patch.js'});
mods.push({name:'verifiedDeviceInstallIndex',path:'apps/tools/rule-repo/releases/test-3.5.6-rc3/verified_install_index_patch.js'});
mods.push({name:'stablePromotionPatch356',path:'apps/tools/rule-repo/releases/3.5.6/stable_patch.js'});
d.id='rule-repo';d.name='我的规则仓库';d.version='3.5.6';d.build=393;d.ref='main';d.modules=mods;d.verify={global:'HikerRuleRepo',property:'version',equals:'3.5.6'};RULE_REPO_CONFIG.defaultRelease=d;
RuleRepoBoot.updatePage=function(){
 setPageTitle('正式版更新');var d=[],m=this.manager(),info=m.info(RULE_REPO_CONFIG),cur=info.current||RULE_REPO_CONFIG.defaultRelease,prev=info.previous||null,cfg=JSON.stringify(RULE_REPO_CONFIG),mgrs=JSON.stringify(MGR_URLS);
 d.push({title:'我的规则仓库',desc:'当前 '+cur.version+' · Build '+cur.build+'\nStable 3.5.6 · Updater 2.0.4 · Verified Install Index 14.2',img:RULE_REPO_ICON,pic_url:RULE_REPO_ICON,col_type:'icon_1_left_pic',url:'hiker://empty',extra:{lineVisible:false}});
