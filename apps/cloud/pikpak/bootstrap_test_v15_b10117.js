/* PikPak Remote Test Bootstrap 0.1.0-test.15 Build10117 - Web var/network bridge */
var PIKPAK_BOOT_CONFIG={id:'pikpak-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',version:'0.1.0-test.15',build:10117,modules:[
...
{name:"pages-test15-meta",path:"apps/cloud/pikpak/releases/0.1.0-test.15-b10117/pages_meta_patch.js"},
...
{name:"runtime-test15-identity",path:"apps/cloud/pikpak/releases/0.1.0-test.15-b10117/runtime_version_patch.js"}
]};
var PikPakBoot={
loadOnly:function(){var root=PIKPAK_BOOT_CONFIG.repoRawRoot+PIKPAK_BOOT_CONFIG.branch+'/',loaded=[],i,m,url;for(i=0;i<PIKPAK_BOOT_CONFIG.modules.length;i++){m=PIKPAK_BOOT_CONFIG.modules[i];url=root+m.path+'?pikpak_release=0.1.0-test.15-b10117';require(url,{headers:{'Cache-Control':'no-cache'}},10117);loaded.push({name:m.name,url:url});}if(typeof PikPakRemoteRuntime==='undefined'||String(PikPakRemoteRuntime.version)!=='0.1.0-test.15'||Number(PikPakRemoteRuntime.build)!==10117)throw new Error('PikPak Test15 Build10117 运行时校验失败');return {ok:true,release:{id:'pikpak-test',version:'0.1.0-test.15',build:10117,variant:'web-session-var-network-bridge'},loaded:loaded};},
module:function(){this.loadOnly();return PikPakRemoteRuntime.module();},
info:function(){return {managerVersion:'direct-loader',current:{id:'pikpak-test',version:'0.1.0-test.15',build:10117,variant:'web-session-var-network-bridge'}};},
check:function(){return {ok:true,current:{id:'pikpak-test',version:'0.1.0-test.15',build:10117},hasUpdate:false};},
