/* PikPak Remote Test Bootstrap 0.1.0-test.11 Build10113 - official Web token handoff */
var PIKPAK_BOOT_CONFIG={id:'pikpak-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',version:'0.1.0-test.11',build:10113,modules:[
...
loadOnly:function(){
    ...
    for(i=0;i<PIKPAK_BOOT_CONFIG.modules.length;i++){
        m=PIKPAK_BOOT_CONFIG.modules[i];url=root+m.path+'?pikpak_release=0.1.0-test.11-b10113';
        require(url,{headers:{'Cache-Control':'no-cache'}},10113);loaded.push({name:m.name,url:url});
    }
    if(typeof PikPakRemoteRuntime==='undefined'||String(PikPakRemoteRuntime.version)!=='0.1.0-test.11'||Number(PikPakRemoteRuntime.build)!==10113)throw new Error('PikPak Test11 Build10113 运行时校验失败');
    return {ok:true,release:{id:'pikpak-test',version:'0.1.0-test.11',build:10113,variant:'official-web-token-handoff'},loaded:loaded};
},
module:function(){this.loadOnly();return PikPakRemoteRuntime.module();},
info:function(){return {managerVersion:'direct-loader',current:{id:'pikpak-test',version:'0.1.0-test.11',build:10113,variant:'official-web-token-handoff'}};},
check:function(){return {ok:true,current:{id:'pikpak-test',version:'0.1.0-test.11',build:10113,variant:'official-web-token-handoff'},hasUpdate:false};},
