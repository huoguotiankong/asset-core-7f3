/* PikPak Remote Test Bootstrap 0.1.0-test.14 Build10116 - official Web drive-entry; Test13 module-path correction */
var PIKPAK_BOOT_CONFIG={id:'pikpak-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',version:'0.1.0-test.14',build:10116,modules:[
...
{name:"runtime-test14-identity",path:"apps/cloud/pikpak/releases/0.1.0-test.14-b10116/runtime_version_patch.js"}
]};
var PikPakBoot={
loadOnly:function(){
    ...
    for(i=0;i<PIKPAK_BOOT_CONFIG.modules.length;i++){
        m=PIKPAK_BOOT_CONFIG.modules[i];url=root+m.path+'?pikpak_release=0.1.0-test.14-b10116';
        require(url,{headers:{'Cache-Control':'no-cache'}},10116);loaded.push({name:m.name,url:url});
    }
    if(typeof PikPakRemoteRuntime==='undefined'||String(PikPakRemoteRuntime.version)!=='0.1.0-test.14'||Number(PikPakRemoteRuntime.build)!==10116)throw new Error('PikPak Test12 Build10116 运行时校验失败');
    return {ok:true,release:{id:'pikpak-test',version:'0.1.0-test.14',build:10116,variant:'official-web-drive-entry'},loaded:loaded};
},
module:function(){this.loadOnly();return PikPakRemoteRuntime.module();},
info:function(){return {managerVersion:'direct-loader',current:{id:'pikpak-test',version:'0.1.0-test.14',build:10116,variant:'official-web-drive-entry'}};},
check:function(){return {ok:true,current:{id:'pikpak-test',version:'0.1.0-test.14',build:10116,variant:'official-web-drive-entry'},hasUpdate:false};},
