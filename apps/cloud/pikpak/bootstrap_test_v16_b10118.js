/* PikPak Remote Test Bootstrap 0.1.0-test.16 Build10118 - drive management */
var PIKPAK_BOOT_CONFIG={id:'pikpak-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',version:'0.1.0-test.16',build:10118,modules:[
...
{name:"runtime-test16-identity",path:"apps/cloud/pikpak/releases/0.1.0-test.16-b10118/runtime_version_patch.js"}
]};
var PikPakBoot={
loadOnly:function(){var root=PIKPAK_BOOT_CONFIG.repoRawRoot+PIKPAK_BOOT_CONFIG.branch+'/',loaded=[],i,m,url;for(i=0;i<PIKPAK_BOOT_CONFIG.modules.length;i++){m=PIKPAK_BOOT_CONFIG.modules[i];url=root+m.path+'?pikpak_release=0.1.0-test.16-b10118';require(url,{headers:{'Cache-Control':'no-cache'}},10118);loaded.push({name:m.name,url:url});}if(typeof PikPakRemoteRuntime==='undefined'||String(PikPakRemoteRuntime.version)!=='0.1.0-test.16'||Number(PikPakRemoteRuntime.build)!==10118)throw new Error('PikPak Test16 Build10118 运行时校验失败');return {ok:true,release:{id:'pikpak-test',version:'0.1.0-test.16',build:10118,variant:'drive-management-video-playlist-my-pack'},loaded:loaded};},
module:function(){this.loadOnly();return PikPakRemoteRuntime.module();},
info:function(){return {managerVersion:'direct-loader',current:{id:'pikpak-test',version:'0.1.0-test.16',build:10118,variant:'drive-management-video-playlist-my-pack'}};},
check:function(){return {ok:true,current:{id:'pikpak-test',version:'0.1.0-test.16',build:10118},hasUpdate:false};},
