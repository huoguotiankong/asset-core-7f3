/* PikPak 0.1.0-test.9 Pages Patch - current txCaptcha completion handoff */
(function(C,UI,Pages){
    ...
    Pages.verify=function(){
        ...
        d.push({title:'请完成 PikPak 官方安全验证',desc:'Test9 已切换到当前 txCaptcha 登录验证契约。完成验证后会监听官方回调、地址栏 token 和页面成功状态，并自动继续登录。',url:'hiker://empty',col_type:'text_1',extra:{lineVisible:false}});
        var interceptor=$.toString(function(original){
            ...
            var challenge=/\/captcha\/v2\/(?:spritePuzzle|reCaptcha|txCaptcha)\.html/i.test(u);
            var loading=/mypikpak\.(com|net)\/loading/i.test(u);
            var callback=/^xlaccsdk01:\/\/xbase\.cloud\/callback/i.test(u);
            if(!tok&&(loading||callback))tok=String(original||'');
            if(tok&&(!challenge||tok!==String(original||''))){
                return "fy_bridge_app.open('hiker://page/pikpakVerifyDone?rule=PikPak&simple=true&captcha_token="+encodeURIComponent(tok)+"')";
            }
            return false;
        },orig);
        var inject="(function(){try{if(window.__pikpakVerifyWatchV9)return;window.__pikpakVerifyWatchV9=1;var original="+JSON.stringify(orig)+";var sent='';function dec(v){try{return decodeURIComponent(v);}catch(e){return v;}}function tokenFrom(u){u=String(u||'');var m=u.match(/[?&#]captcha_token=([^&#]+)/i);return m?dec(m[1]):'';}function currentToken(){var t=tokenFrom(String(location.href||''));if(t&&t!==original)return t;try{var fs=document.getElementsByTagName('iframe');for(var i=0;i<fs.length;i++){var s=tokenFrom(fs[i].src||'');if(s&&s!==original)return s;try{s=tokenFrom(String(fs[i].contentWindow.location.href||''));if(s&&s!==original)return s;}catch(e2){}}}catch(e3){}return t||'';}function successState(){try{var x=String((document.body&&document.body.innerText)||'').replace(/\\s+/g,'').toLowerCase();return x.indexOf('验证成功')>=0||x.indexOf('验证完成')>=0||x.indexOf('验证通过')>=0||x.indexOf('verificationcomplete')>=0||x.indexOf('verificationsuccess')>=0||x.indexOf('verified')>=0||x.indexOf('passed')>=0;}catch(e){return false;}}function handoff(tok){tok=String(tok||original||'');if(!tok||sent===tok)return;sent=tok;try{fy_bridge_app.open('hiker://page/pikpakVerifyDone?rule=PikPak&simple=true&captcha_token='+encodeURIComponent(tok));}catch(e){sent='';}}function probe(){try{var href=String(location.href||''),t=currentToken();if(/^xlaccsdk01:\/\/xbase\.cloud\/callback/i.test(href)){handoff(t||original);return;}if(t&&t!==original){handoff(t);return;}if(successState())handoff(t||original);}catch(e){}}probe();window.__pikpakVerifyTimerV9=setInterval(probe,350);try{window.addEventListener('hashchange',probe);window.addEventListener('popstate',probe);document.addEventListener('readystatechange',probe);}catch(e4){}}catch(e){}})();";
        ...
        d.push({title:'验证完成，继续登录',desc:'自动接回未触发时点这里；会直接用本次已验证 captcha token 再执行一次 signin。',url:$("#noLoading#").lazyRule(function(){return $.require('pikpak').verifyContinueAction();}),col_type:'text_center_1',extra:{lineVisible:false}});
