/* PikPak Remote Test Bootstrap 0.1.0-test.8 - immutable direct loader */
var PIKPAK_BOOT_CONFIG={id:'pikpak-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',version:'0.1.0-test.8',build:10108,modules:[
...
loadOnly:function(){
    ...
    for(i=0;i<PIKPAK_BOOT_CONFIG.modules.length;i++){
        m=PIKPAK_BOOT_CONFIG.modules[i];url=root+m.path+'?pikpak_release=0.1.0-test.8';
        require(url,{headers:{'Cache-Control':'no-cache'}},10108);loaded.push({name:m.name,url:url});
    }
    if(typeof PikPakRemoteRuntime==='undefined'||String(PikPakRemoteRuntime.version)!=='0.1.0-test.8'||Number(PikPakRemoteRuntime.build)!==10108)throw new Error('PikPak Test8 运行时校验失败');
    return {ok:true,release:{id:'pikpak-test',version:'0.1.0-test.8',build:10108},loaded:loaded};
},
module:function(){this.loadOnly();return PikPakRemoteRuntime.module();},
info:function(){return {managerVersion:'direct-loader',current:{id:'pikpak-test',version:'0.1.0-test.8',build:10108}};},
check:function(){return {ok:true,current:{id:'pikpak-test',version:'0.1.0-test.8',build:10108},hasUpdate:false};},
