...
## 2026-09-24 · 0.1.0-test.24 / Build10126 · 流畅转码首选与可诊断播放

- Test23 海阔实机反馈：首页顶部状态被挤成一行，最近三个 Magnet 重复且首屏太长；会员播放和拖动依然明显慢于 PikPak 原生 APP。单纯调换 `medias` 与原文件顺序未能证明速度改善。
- 首页改用紧凑 `text_1` 标题与容量状态，不再使用实机截断的 `avatar`；四个主入口、四个次级入口、链接输入和文件列表直接衔接，移除重复的最近输入按钮。
- 默认选“流畅转码优先”：优先专用 HLS 与约 1080P/720P 的转码线路，保留原画、官方原顺序及原文件兼容模式可切换；并不声称已复制原生 APP 的缓存/播放器能力。
- 设置页显示最近一次播放的 API 取链耗时、来源、默认线路名与 HLS/直链类型。只保存无敏感字段的诊断，不记录真实 URL、Access/Refresh Token。
- 不改 Test22 已实机确认的单账号官方 Web 登录，不动 Stable；播放首帧、远端 seek 与 Magnet 需海阔实机验证，并在结果出来后决定是否进一步改播放器交接层。

当前 Test：0.1.0-test.24 / Build10126；Stable 尚未建立。

## 2026-09-24 · 0.1.0-test.25 / Build10127 · 回退播放器交接回归

- 实机确认 Test24 视频出现 0 kb/s 与「播放异常」，属于必须回退的播放回归；截图未提供响应详情，不能断言具体 CDN 或授权失败。
- 对照代码发现 Test24 对每条 URL 附加 `#isVideo=true#` 的同时又在整个多线路 JSON 后附加相同标记；此前 Test23 仅在线路 URL 上附加。Test24 还默认重排画质并移除了 Test23 的短时详情缓存，三项都不应在缺少播放器实机验证时改变。
- Test25 将播放模块恢复为 Test23 原文件、原顺序、原多线路 JSON 输出及三分钟内存详情缓存；设置恢复「官方流媒体优先 / 原文件优先」两项，保留 Test24 紧凑首页，账号继续单 Web Session。Test24 诊断与未验证的流畅转码默认线路暂时移除。
- 静态语法、模块加载链与播放器输出合同先检查；仍需实机验证既有视频、拖动及 Magnet 临时视频。未验证不升 Stable。

当前 Test：0.1.0-test.25 / Build10127；Stable 尚未建立。
