...
var cid=q("cid","0"),cname=q("cname","我的文件"),key="115BatchSelV1_"+cid,pageNo=1;try{pageNo=parseInt(MY_PAGE||1,10)||1;}catch(e4){}
var selected=ops.load(key),selCount=ops.count(selected);
if(pageNo===1){
 d.push({title:"☑ 批量管理 · "+cname,desc:"点击条目选择/取消；复制和移动会进入目标文件夹选择页",col_type:"text_1",extra:{lineVisible:false}});
...
if(pageNo===1){
 ...
 d.push({title:"复制",col_type:"text_3",url:$().lazyRule(function(selKey,sourceCid,sourceName){try{var o=$.require("115BatchOps"),n=o.count(o.load(selKey));if(!n)return "toast://请先选择文件或文件夹";return "hiker://page/115FolderPicker?rule=115.简&page=fypage&op=copy&selKey="+encodeURIComponent(selKey)+"&sourceCid="+encodeURIComponent(sourceCid)+"&sourceName="+encodeURIComponent(sourceName)+"&cid=0&cname="+encodeURIComponent("根目录");}catch(ex){return "toast://打开目标目录失败："+String(ex.message||ex);}},key,cid,cname)});
 d.push({title:"移动",col_type:"text_3",url:$().lazyRule(function(selKey,sourceCid,sourceName){try{var o=$.require("115BatchOps"),n=o.count(o.load(selKey));if(!n)return "toast://请先选择文件或文件夹";return "hiker://page/115FolderPicker?rule=115.简&page=fypage&op=move&selKey="+encodeURIComponent(selKey)+"&sourceCid="+encodeURIComponent(sourceCid)+"&sourceName="+encodeURIComponent(sourceName)+"&cid=0&cname="+encodeURIComponent("根目录");}catch(ex){return "toast://打开目标目录失败："+String(ex.message||ex);}},key,cid,cname)});
}
