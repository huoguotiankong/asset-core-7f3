...
if(pageNo===1){
 ...
 d.push({title:"☑ 批量",col_type:"text_4",url:"hiker://page/115FileBatch?rule=115.简&page=fypage&cid="+encodeURIComponent(cid)+"&cname="+encodeURIComponent(cname)});
...
for(var i=0;i<arr.length;i++){
 ...
 item.extra.longClick.push({title:"复制到…",js:$.toString(function(id,nm,dir,sz,sourceCid,sourceName){try{var o=$.require("115BatchOps"),key="115SingleSelV1_"+String(id),m={};m[String(id)]={name:String(nm||""),dir:!!dir,size:Number(sz||0)};o.save(key,m);return "hiker://page/115FolderPicker?rule=115.简&page=fypage&op=copy&selKey="+encodeURIComponent(key)+"&sourceCid="+encodeURIComponent(sourceCid)+"&sourceName="+encodeURIComponent(sourceName)+"&cid=0&cname="+encodeURIComponent("根目录");}catch(ex){return "toast://打开复制目标失败："+String(ex.message||ex);}},fid,name,isDir,Number(f.size||0),cid,cname)});
 item.extra.longClick.push({title:"移动到…",js:$.toString(function(id,nm,dir,sz,sourceCid,sourceName){try{var o=$.require("115BatchOps"),key="115SingleSelV1_"+String(id),m={};m[String(id)]={name:String(nm||""),dir:!!dir,size:Number(sz||0)};o.save(key,m);return "hiker://page/115FolderPicker?rule=115.简&page=fypage&op=move&selKey="+encodeURIComponent(key)+"&sourceCid="+encodeURIComponent(sourceCid)+"&sourceName="+encodeURIComponent(sourceName)+"&cid=0&cname="+encodeURIComponent("根目录");}catch(ex){return "toast://打开移动目标失败："+String(ex.message||ex);}},fid,name,isDir,Number(f.size||0),cid,cname)});
