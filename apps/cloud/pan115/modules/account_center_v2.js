...
function target(){var x="0",xn="",h="0",c="0";try{x=String(getItem("115OfflineCustomCid","0")||"0");xn=String(getItem("115OfflineCustomName","")||"");}catch(e){}try{h=String(getItem("115HikerVisionCid","0")||"0");}catch(e2){}try{c=String(getItem("115CloudDownloadCid","0")||"0");}catch(e3){}if(x!=="0")return{id:x,name:xn||"自定义目录",mode:"自定义"};if(h!=="0")return{id:h,name:"海阔视界",mode:"自动"};if(c!=="0")return{id:c,name:"云下载/离线下载",mode:"自动"};return{id:"0",name:"根目录",mode:"自动"};}
if(!client){d.push({title:"👤 115账号未连接",desc:loginErr||"请先完成登录",col_type:"text_center_1"});d.push({title:"登录 / 账号管理",col_type:"text_1",url:"hiker://page/115Account?rule=115.简&page=fypage"});setResult(d);return;}
var user=null,info=null,userErr="",infoErr="";
try{if(typeof client.getUser==="function")user=client.getUser();}catch(e4){userErr=String(e4.message||e4);}
try{if(typeof client.getInfo==="function")info=client.getInfo();}catch(e5){infoErr=String(e5.message||e5);}
...
var tg=target();
d.push({title:"离线保存目录",desc:tg.name+" · "+tg.mode+" · 点击更改",col_type:"text_1",url:"hiker://page/115OfflineTargetPicker?rule=115.简&page=fypage&cid=0&cname="+encodeURIComponent("我的文件")+"&trail="+encodeURIComponent(JSON.stringify([{id:"0",name:"我的文件"}])),extra:{lineVisible:false}});
