...
if(pg===1){
 d.push({title:'<b>⬇ 115 离线下载中心</b>'.fontcolor("#2B6CB0"),col_type:"rich_text",extra:{textSize:19,lineVisible:false}});
 ...
 d.push({title:"📁 保存目录："+target.name,desc:target.id!=="0"?"点击打开保存目录":"首次提交时自动识别保存目录",col_type:"text_1",url:target.id!=="0"?("hiker://page/115FileManage?rule=115.简&page=fypage&cid="+encodeURIComponent(target.id)+"&cname="+encodeURIComponent(target.name)):"toast://首次提交任务时会自动识别保存目录",extra:{lineVisible:false}});
...
if(!shown&&pg===1)d.push({title:all.length?"当前没有符合筛选的任务":"暂无离线任务",desc:all.length?"切换上方筛选即可":"可在上方粘贴链接创建任务",col_type:"text_center_1"});
setResult(d);
})();
