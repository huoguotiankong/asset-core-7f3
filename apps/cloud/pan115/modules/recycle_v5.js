...
function recycleArray(raw){if(raw instanceof Array)return raw;if(raw&&raw.data instanceof Array)return raw.data;if(raw&&raw.items instanceof Array)return raw.items;if(raw&&raw.list instanceof Array)return raw.list;return [];}
var p=1;try{p=parseInt(MY_PAGE||1,10)||1;}catch(e3){}
if(p===1){
 d.push({title:"🗑 回收站",desc:"点击条目还原 · 长按永久删除",col_type:"text_1",extra:{lineVisible:false}});
 d.push({title:"↻ 刷新",col_type:"text_3",url:$("刷新").lazyRule(function(){refreshPage(false);return "hiker://empty";})});
 d.push({title:"📁 文件管理",col_type:"text_3",url:"hiker://page/115FileManage?rule=115.简&page=fypage&cid=0&cname="+encodeURIComponent("我的文件")});
 d.push({title:"🧹 清空回收站",col_type:"text_3",url:$("","清空回收站不可恢复 · 请输入115安全密码").input(function(){
  var pwd=String(input||"").trim();if(!pwd)return "toast://已取消";
  return $("确认清空整个115回收站？\n所有内容都会永久删除，且无法恢复").confirm(function(password){
   var deleted=0,total=0;
   try{
    var api2=$.require("115Api"),c=api2.newClient();
    if(!c||typeof c.cleanRecycleBin!=="function")return "toast://当前115Api未提供清空回收站方法";
    function lr(off,lim){var fn=null;try{if(c&&typeof c.listRecycleBin==="function")fn=c.listRecycleBin;}catch(e0){}if(!fn)try{if(c&&typeof c.getRecycleBin==="function")fn=c.getRecycleBin;}catch(e1){}if(!fn)throw new Error("当前115Api未提供回收站列表方法");return fn.call(c,Number(off||0),Number(lim||40));}
    function arr(raw){if(raw instanceof Array)return raw;if(raw&&raw.data instanceof Array)return raw.data;if(raw&&raw.items instanceof Array)return raw.items;if(raw&&raw.list instanceof Array)return raw.list;return [];}
    function ck(r){if(r&&r.state===false)throw new Error(r.error||r.msg||"115返回失败");if(r&&r.code!==undefined&&Number(r.code)!==0&&Number(r.code)!==200)throw new Error(r.message||r.msg||("115返回code="+r.code));return r;}
    var ids=[],seen={},offset=0,limit=40,guard=0,overflow=false;
    while(guard<200){
     var page=arr(lr(offset,limit));
     if(!page.length)break;
     for(var i=0;i<page.length;i++){
      var x=page[i]||{},rid=String(x.fileId||x.file_id||x.id||x.rid||"");
      if(rid&&!seen[rid]){seen[rid]=1;ids.push(rid);}
     }
     offset+=page.length;guard++;
     if(page.length<limit)break;
     if(guard>=200)overflow=true;
    }
    if(overflow)throw new Error("回收站项目过多，安全保护已停止操作，请先分批清理后重试");
    total=ids.length;
    if(!total){refreshPage(false);return "toast://回收站已经为空";}
    var batchSize=40;
    for(var z=0;z<ids.length;z+=batchSize){var part=ids.slice(z,z+batchSize);ck(c.cleanRecycleBin(String(password),part));deleted+=part.length;}
    refreshPage(false);return "toast://已清空回收站，共永久删除"+deleted+"项";
   }catch(ex){try{refreshPage(false);}catch(er){}return "toast://清空失败"+(deleted?"（已删除"+deleted+"/"+total+"项）":"")+"："+String(ex.message||ex);}
  },pwd);
 })});
}
