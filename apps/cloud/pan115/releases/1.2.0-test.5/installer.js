...
// 始终从已实机验证 Stable 1.1.0 磁链模块重建，避免历史 Test 补丁叠加。
...
var helper=`
/* 1.2.0-test.5 in-place：海阔视界目录优先；云下载兜底 */
...
function _115SavedHikerCid(){
    try{
        let x=String(getItem("115HikerVisionCid","0")||"0");
        return x&&x!=="0"?x:"0";
    }catch(e){return "0";}
}
...
function _115ResolveTargetCid(c){
    ...
    try{
        ...
        for(let i=0;i<a.length;i++){
            ...
            let n=String(f.name||"").trim();
            let cid=String(f.fileId||"");
            if(!cid) continue;
            if(n==="海阔视界"){
                try{
                    setItem("115HikerVisionCid",cid);
                    setItem("115OfflineTargetMode","hiker-root");
                }catch(e1){}
                return cid;
            }
    ...
    if(cloudFound){
        try{
            setItem("115CloudDownloadCid",cloudFound);
            setItem("115OfflineTargetMode","cloud-root-fallback");
        }catch(e3){}
        return cloudFound;
    }
...
offlineRule=offlineRule.replace(/c\.addOfflineTaskURIs\(\[link\],\s*["']0["']\)/g,'_115ResolveTargetCid(c); c.addOfflineTaskURIs([link], _115SavedTargetCid())');
offlineRule=offlineRule.replace(/c\.addOfflineTaskURIs\(add,\s*["']0["']\)/g,'_115ResolveTargetCid(c); c.addOfflineTaskURIs(add, _115SavedTargetCid())');
...
rule.title="115.简";
rule.author="AI&三鲜汤 · 海阔视界目录优先 1.2.0-test.5";
rule.version=2026092206;
try{
...
var out="hiker://files/cache/115_12005_hiker_folder.json";
