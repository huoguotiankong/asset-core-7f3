...
状态：**Stable 1.2.0 / Build 2026092220 / 已实机验证；Test 1.2.1-test.15 / Build 2026092235 / 待实机验证**  
首次纳入：2026-09-21  
最近更新：2026-09-22

## 1.2.1-test.15 / Build 2026092235 — 离线中心 Phase 2

用户要求继续按既定规划优化。Test14 的“搜索/输入框短提示 + 技术信息降级”继续作为 UI 基线，但本轮没有收到 Test14 的独立实机结论，因此不虚构 `deviceValidated`；安装器同时接受 Test13 / Test14 直接覆盖。

### 本轮产品目标

继续只优化离线中心，不碰已经闭环的搜索、文件管理、回收站和外部磁链安全链：

- 输入框继续只显示 `粘贴链接`；
- 移除页内重复“115 离线下载中心”标题，系统标题栏负责页面身份；
- 保存目录行只显示用户能理解的目录名、任务总数/页数，不暴露 CID / 内部 mode；
- 任务行只显示真正有用的状态：完成/下载中/失败/等待、进度、速度、剩余时间、大小、时间；
- `info_hash`、原链接等技术信息继续放到长按菜单；
- 已完成任务增加**保守单视频一步播放**：若结果本身就是视频，或结果目录顶层只有 1 个非 sample/trailer/广告类视频，则点击任务直接播放；
- 多视频、嵌套目录或无法安全判断时，继续进入既有 `115OfflineResult` 智能结果页，不重写已有扫描逻辑。

### 协议依据

复核 115Driver 的 `OfflineTask` 合同，已确认下列字段存在：

```text
info_hash / name / size / url
add_time / last_update / left_time
rateDownload / status / percentDone
file_id / wp_path_id
```

当前 115Api 已经把核心字段规范化为 `infoHash / fileId / dirId / rateDownload / percent ...`，本轮只消费这些现有字段，不新增猜测接口。
...
- 禁止后台自动轮询；
- 禁止并发轮询；
- 仅在用户明确点击“已完成任务”时，最多执行 `getFile + 1 次顶层 getFiles` 来判断是否可单视频直放；
- 判断失败立即回落现有结果页；
- 外部 `115Offline?add=` 的低频 `focusMode` 完全不动；
- Stable 1.2.0 不动。

模块：`apps/cloud/pan115/modules/offline_center_v3.js`  
安装器：`apps/cloud/pan115/releases/1.2.1-test.15/installer.js`  
状态：`pending-device-validation-offline-center-phase2`。

### Test15 实机验收

1. 输入框仍只显示短提示，不出现协议说明；
2. 保存目录行不出现 CID / hiker-cache；
3. 普通任务行不显示 raw info_hash；
4. 下载中任务若 115 返回数据，应能看到进度 / 速度 / 剩余时间；
5. 已完成任务应显示大小和紧凑时间；
6. 单视频任务点击应尽量一步播放；
7. 多视频任务仍进入原智能结果页；
8. 长按仍可复制原链接 / info_hash、移除任务记录，失败任务可重试；
9. 搜索、文件管理、回收站、外部磁链播放不得回归。
...
状态：`pending-device-validation-superseded`，未单独实机验证即由 Test15 接手。
...
1. **离线中心 Phase 2**：状态、人性化时间、长按菜单、单视频快速播放；
...
Stable 1.2.0 不动。搜索“能搜到 + 视频播放 + 关键词显示 + 目录跳转”已闭环。当前 Test15 只做 **离线中心 Phase 2**；单视频快速播放只能是显式点击时的保守快路径，多视频继续复用既有结果页。Test15 实机通过后再进入 **账号 / 容量 / 设置中心**，不要跨多个模块同时大改。
