...
状态：**Stable 1.2.0 / Build 2026092220 / 已实机验证；Test 1.2.1-test.4 / Build 2026092224 / 待实机验证**  
首次纳入：2026-09-21  
最近更新：2026-09-22

## 1.2.1-test.4 / Build 2026092224 — 文件管理导航 / 排序筛选 / 文件信息 / 目标目录体验

用户要求继续优化文件管理。当前没有收到对 Test3 新建文件夹、重命名、单项快捷复制/移动的明确“实机通过”反馈，因此 Test3 不标记 deviceValidated；Test4 继续保持 Test 通道，不进入 Stable。

本轮不继续扩张 115 协议层，重点优化文件管理产品体验，并保持已经验证的 `115BatchOps`、回收站、磁链播放和播放器链不变。
...
→ 面包屑路径提示
→ 根目录 / 上一级显式导航
→ 名称↑ / 名称↓ / 时间↓ / 时间↑ / 大小↓ / 大小↑ 排序循环
→ 全部 / 文件夹 / 视频 / 图片 / 音频 / 文档 / 压缩包 / 其它 类型筛选
→ 长按“文件信息”查看 ID / PickCode / SHA1 / 大小 / 时间等
→ 批量管理后保持原目录上下文
...
- 排序仍由现有 `client.getFiles()` 传递 `file_name / user_utime / file_size + asc`，不新增认证请求；
- 类型筛选仅作用于当前分页返回的文件集合，不修改 115 服务端接口；
- 文件信息页优先调用现有 `client.getFile(fileId)`，失败时使用列表页已经携带的名称/大小/PickCode 作为降级数据；
- 单项删除在 V8 中统一复用 Test2 已验证的 `115BatchOps.exec("delete")`，不再保留重复的删除 request 改写代码；
- 新版目标目录页仍调用原 `115BatchOps.exec(copy/move)`，只增加导航、最近目标和目录内新建；
- 继续禁止并发批量请求，不改登录、Cookie、m115、离线、磁链、播放器、回收站清空合同。

模块快照：

- commit：`9529ab752c3937bf3ee8f9bde16afc32705d59c7`
- 文件管理：`apps/cloud/pan115/modules/file_manage_v8.js`
- 文件信息：`apps/cloud/pan115/modules/file_info_v1.js`
- 新建/重命名核心继续：`apps/cloud/pan115/modules/file_ops_v1.js`
- 批量核心继续：`apps/cloud/pan115/modules/batch_ops_v1.js`
- 批量页：`apps/cloud/pan115/modules/file_batch_v2.js`
- 目标目录页：`apps/cloud/pan115/modules/folder_picker_v2.js`
- 回收站继续：`apps/cloud/pan115/modules/recycle_v5.js`
- Release：`apps/cloud/pan115/releases/1.2.1-test.4/release.json`

实机验收优先顺序：

1. 文件管理进入多层目录后，“根目录 / 上一级 / 路径提示”应正确；
2. 连续点击排序按钮，验证名称、时间、大小升降序都能正常返回列表；
3. 连续切换筛选按钮，至少验证文件夹、视频、图片；
4. 长按文件/文件夹 → 文件信息，确认信息页能打开，文件 ID 可复制；视频仍能从信息页播放；
5. 进入批量管理再复制/移动，完成后应回到原目录而不是丢失目录上下文；
6. 目标目录选择器验证根目录、上一级、新建文件夹、最近目标；
7. 原新建、重命名、批量删除/复制/移动、回收站清空、磁链播放、登录和“我的文件”不得回归。

当前状态：`pending-device-validation-file-manager-ux`。
...
- Test3 已被 Test4 覆盖，但本轮没有收到明确实机通过反馈，因此不标记 deviceValidated。
...
Stable 1.2.0 仍是日常恢复基线；Test1 清空回收站、Test2 批量删除/复制/移动均已实机通过。Test3 未收到明确设备通过反馈，Test4 在其基础上继续做文件管理 UX；Test4 实机通过前不得进入 Stable。后续进一步优化优先放在搜索、批量选择体验、文件属性与目录导航，不得为了文件管理改动登录、磁链播放或原播放器链。