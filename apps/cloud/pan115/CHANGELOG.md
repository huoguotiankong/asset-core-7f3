...
状态：**Stable 1.2.0 / Build 2026092220 / 已实机验证；Test 1.2.1-test.2 / Build 2026092222 / 待实机验证**  
首次纳入：2026-09-21  
最近更新：2026-09-22

## 1.2.1-test.2 / Build 2026092222 — 文件批量删除 / 复制 / 移动
...
1. 进入文件管理，原文件夹浏览、视频点击、单文件长按删除仍正常；
2. 进入“批量管理”，选择/取消、已选数量、全选本页、清空选择正常；
3. 用 2～3 个可牺牲测试文件执行批量删除，确认全部进入回收站；
4. 用 1～2 个测试文件复制到另一个文件夹，源文件保留、目标目录出现副本；
5. 用 1～2 个测试文件移动到另一个文件夹，源目录消失、目标目录出现；
6. 文件夹也分别测试一次复制/移动；
7. 回收站清空、磁链播放、登录与“我的文件”不得发生回归。

当前状态：`pending-device-validation-batch-file-management`。
...
Stable 1.2.0 是新的日常恢复基线。后续文件管理能力继续从 Stable/Test 已验证链派生并小步实机验证；批量删除、复制、移动在 Test2 验证通过前不得进入 Stable。后续“新建文件夹、重命名”等继续单独 Test，不得为了文件管理改动登录、磁链播放或原播放器链。
