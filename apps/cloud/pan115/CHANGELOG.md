...
状态：**Stable 1.2.0 / Build 2026092220 / 已实机验证；Test 1.2.1-test.13 / Build 2026092233 / 待实机验证**  
首次纳入：2026-09-21  
最近更新：2026-09-22
...
状态：`pending-device-validation-offline-center-phase1`。

### Test13 实机验收

1. 原搜索、目录跳转和视频播放不得回归；
2. 从原“离线任务”页能看到“离线下载中心（新版）”入口；
3. 离线中心能正常列出任务，并可切换 4 种筛选；
4. 手动刷新能更新状态，页面不会自己持续请求；
5. 新建磁链任务仍保存到预期目录；
6. 已完成任务能进入现有结果页；
7. 长按移除任务记录后，已下载文件仍保留。
...
Stable 1.2.0 不动。搜索“能搜到 + 视频播放 + 关键词显示 + 目录跳转”已闭环。当前 Test13 只做 **离线下载中心 Phase 1**，外部磁链的安全低频 focusMode 与 Stable 1.2.0 保持不动；实机通过后再继续账号/容量/设置、收藏/历史、分类浏览及全局空态/错误态等后续规划。
