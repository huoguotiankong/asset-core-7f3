...
}catch(e3){return "toast://下载 Test4 模块失败："+e3.message;}
if(!offlineRule||offlineRule.indexOf("115Offline")<0) return "toast://Test4 离线模块为空";
if(!resultRule||resultRule.indexOf("115OfflineResult")<0) return "toast://Test4 选集模块为空";
...
rule.title="115.简·测试";
rule.author="AI&三鲜汤 · 磁链播放增强 Test4";
rule.version=2026092109;
try{
  ...
  fr=fr.replace('"hiker://page/115Search?rule=" + rule + "&page=fypage&kw=" + encodeURIComponent(url);','"hiker://page/115Offline?rule=" + rule + "&page=fypage&add=" + encodeURIComponent(url);');
  rule.find_rule=fr;
}catch(e4){}
rule.pages=JSON.stringify(pages);
var out="hiker://files/cache/115_test4_rule.json";
