/* JavMenu Playback Adapter 0.1.0-test.2 */
var JavMenuPlayback=(function(){
  var P={version:'0.1.0-test.2',build:10102};
  ...
  P.site=function(code,detailUrl,selected){var c=C(),url=detailUrl||c.detailUrl(code),u=c.abs(selected,url);if(u&&!c.isPreviewMedia(u,'','selected'))return P.media(u,url);var h=c.fetchNetwork(url,9000),list=c.parsePlayer(h,url);if(!list.length){var wh=c.fetchWeb(url);list=c.parsePlayer(wh,url);}var out=P.model(list,url);if(out){c.diag('play-direct',url,true,'sources='+list.length);return out;}c.diag('play-fallback',url,false,'no structured full source');return'video://'+url;};
