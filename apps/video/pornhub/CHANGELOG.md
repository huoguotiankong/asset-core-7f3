...
### 第五轮实机结论
- Test5 实机确认：评论页已真正恢复，可读取作者头像、用户名、时间与评论正文；详情创作者头像也已从占位图恢复为真实频道头像；独立搜索页和中文分类页均能正常进入。
- Test5 分类热门双列卡已经接近官网，但卡片标题仍把 `149,030 视频` 等数量拼入分类名；搜索筛选仍因 `scroll_button` 横向溢出出现尾部 `>`；视频标题还存在 `I&#039;m` 这类数字 HTML 实体未解码。
- Test5 Shorts `/shorties` 页面可打开，但旧 `parseShortCards()` 只接受 `/short/` 链接，当前页面未命中，因此显示“Shorts 暂不可用”。
- 用户明确要求：首页登录后默认展示自己的账号推荐内容，并把收藏/历史等高频入口提升到首屏，同时继续压缩 UI 占位并增强整体功能。

### 首页 / 账号 Workspace 重构
- 登录状态下首页默认 `ph_home_feed6=recommended`，直接走当前 X5 会话 `/recommended`；未登录则默认公开“最新”。
- 首页 Feed 改为同页 `为你推荐 / Feed / 最新 / 热门 / 高分`，全部 `putMyVar → refreshPage(false)`，不增加返回栈。
- 若账号推荐/Feed 暂时解析为空，首页不会整页空白，而是回退公开最新内容并明确标记“账号推荐暂未解析，已回退公开内容”。
- 删除 Test5 大号 `movie_1_left_pic` 品牌卡，改为紧凑 `avatar` 品牌/账号状态行。
- `站内收藏 / 观看历史 / 本地收藏 / 本地足迹` 提升为首屏四个高频图标入口；搜索、分类、创作者、订阅、片单、Shorts、GIF、我的改为可换行辅助标签，减少首屏空耗。
- “我的账号”页删除大号账号占位卡和长段诊断说明，改为真实账号头像优先 + 紧凑功能标签 + 三栏账号工具（官方账号 / 重新同步 / 退出会话）。

### Shorts / 搜索 / 分类 / 文本
- Shorts Adapter 改成双链：先解析 `/short/`；没有结果时继续把 `/shorties` 中普通 `view_video.php?viewkey=` 卡片当作短视频列表，进入现有原生详情/多画质播放链。
- 搜索排序、制作类型、时长和范围标签改用可换行 `flex_button`，去掉 Test5 横向滚动尾部 `>`。
- 创作者搜索补齐二级范围：演员 / 频道 / 模特 / 用户，避免“创作者搜索”实际上只查 Pornstars。
- 分类热门卡统一清洗中英文“数字 + 视频/Videos”计数，不再把视频数量混入分类名称；“所有分类”说明改为更简洁的“共 N 个”。
- Core 数字实体解码补齐 `&#039; / &#x27;` 等形式；详情、评论和列表标题统一经过新 decoder。
- 详情分类改为 `flex_button` 换行标签，避免横向分类栏尾部再出现 `>`；评论页把长视频标题下沉到说明，不再占据主标题位置。

### 已验证链保持
- Test5 已实机确认的评论读取、详情创作者真实头像、中文分类和独立搜索继续作为基线。
- Test1/Test2 已实机确认的 4 档 HLS 多画质播放链未改。
- Test4 的 X5 账号会话、Cookie 指纹隔离、在线收藏/订阅合同保持不变。

### 回归门禁
- Test6 `core_patch.js / ui_patch.js / Bootstrap` 已通过 `node --check`。
- Shell JSON 已验证 20 个页面声明全部指向 Build10106，并继续包含评论与片单详情。
- Runtime smoke：`module().home/comments` 均可导出；`I&#039;m → I'm`；Shorts 样例在 `/short/` 未命中时可通过普通 `view_video` 卡片回退。
- Test6 仍只进入 Test；首页账号推荐是否确实对应当前账号、Shorts 当前真实页面能否命中回退、以及新版首屏密度需要继续实机验收。
