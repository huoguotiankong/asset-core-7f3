/* 黄豆短剧 Detail / Topic / Settings 1.9.0-test.1 */
var HuangDouDetailV190=(function(){
  var U=HuangDouUIV190,D=U.design;
  function detailUrlParam(){return U.qp('url','');}
  function groupSize(){var n=parseInt(getItem('hddj_ep_group_v190','30')||'30');if(n<20)n=20;if(n>60)n=60;return n;}
  function detail(c,p){
    var d=[],url=detailUrlParam();
    if(!url){setPageTitle('详情加载失败');setResult([U.empty('缺少详情地址','请返回列表重新进入')]);return;}
    try{
      ...
      var free=null;for(var i=0;i<x.eps.length;i++)if(!x.eps[i].locked){free=x.eps[i];break;}
      var last=parseInt(getItem('hddj_last_'+x.id,'0')||'0'),target=last>0?last:(free?free.no:0),fav=c.isFav(x.url);
      d.push({title:target?(last>0?'继续第 '+last+' 集':'立即播放'):'暂无免费集',url:target?$('#noLoading#').lazyRule(function(id,ep){return $.require('hddj').play(id,ep);},x.id,target):'hiker://empty',col_type:'text_2',extra:{backgroundColor:target?D.active:'',lineVisible:false}});
      d.push({title:fav?'已收藏':'收藏',url:$('#noLoading#').lazyRule(function(obj){var on=$.require('hddj').toggleFav(obj);refreshPage(false);return'toast://'+(on?'已加入收藏':'已取消收藏');},{title:x.title,img:x.img,url:x.url,heat:x.heat,ep:x.meta,tags:[]}),col_type:'text_2',extra:{lineVisible:false}});
      ...
      if(x.eps.length){
        d.push(U.line());d.push(U.section(c,'选集','共 '+x.eps.length+' 集'));
        var reverse=getItem('hddj_reverse_v190','0')==='1',eps=x.eps.slice();if(reverse)eps.reverse();
        d.push(U.chip(reverse?'倒序':'正序',true,$('#noLoading#').lazyRule(function(){setItem('hddj_reverse_v190',getItem('hddj_reverse_v190','0')==='1'?'0':'1');refreshPage(false);return'hiker://empty';}),'text_4'));
        ...
        show.forEach(function(ep){var current=last===ep.no,label=ep.locked?('第'+ep.no+'集 · 锁'):('第'+ep.no+'集');d.push({title:label,url:ep.locked?('webRule://'+c.host()+'/watch/details/'+x.id+'.html?ep='+ep.no):$('#noLoading#').lazyRule(function(id,no){return $.require('hddj').play(id,no);},x.id,ep.no),col_type:'text_4',extra:{backgroundColor:current?D.active:'',lineVisible:false}});});
      }
  ...
  function settings(c,p){
    ...
    d.push(U.line());d.push(U.section(c,'播放','结构化 Token 播放'));
    var st=p.strategy(),name=st==='legacy'?'稳定旧链':(st==='web'?'网页兼容':'智能直连');
    d.push({title:'播放策略',desc:name,url:$('#noLoading#').lazyRule(function(){var ops=['智能直连','稳定旧链','网页兼容'];var now=getItem('hddj_play_strategy_v2','smart'),idx=now==='legacy'?1:(now==='web'?2:0);return'select://'+JSON.stringify({title:'播放策略',options:ops,selectedIndex:idx,col:1,js:$.toString(function(){var m=input==='稳定旧链'?'legacy':(input==='网页兼容'?'web':'smart');setItem('hddj_play_strategy_v2',m);refreshPage(false);})});}),col_type:'text_1'});
    var dg=p.diag(),summary=dg&&dg.stage?(String(dg.stage)+' · '+String(dg.route||'')+(dg.ep?' · 第'+dg.ep+'集':'')):'暂无播放记录';d.push({title:'最近播放诊断',desc:summary,url:'toast://'+('stage='+(dg.stage||'NONE')+' route='+(dg.route||'')+(dg.ep?' ep='+dg.ep:'')+(dg.error?' error='+dg.error:'')),col_type:'text_1'});
    ...
    d.push({title:'恢复 1.9 默认设置',url:$('#noLoading#').lazyRule(function(){clearItem('hddj_col_v190');clearItem('hddj_ep_group_v190');clearItem('hddj_reverse_v190');clearItem('hddj_play_strategy_v2');clearMyVar('hddj_home_tab_v190');clearMyVar('hddj_library_cat_v190');clearMyVar('hddj_mine_tab_v190');refreshPage(false);return'toast://已恢复默认设置';}),col_type:'text_center_1'});
    d.push({title:'版本 1.9.0-test.1',desc:'Native UI / 独立片库搜索专题我的设置 / PlaybackAdapter / 播放诊断 / 手动线路自检',url:'hiker://empty',col_type:'long_text'});setResult(d);
  }
  return{version:'1.9.0-test.1',detail:detail,topic:topic,settings:settings};
})();
