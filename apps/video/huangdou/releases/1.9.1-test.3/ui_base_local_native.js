/* 黄豆短剧 1.9.1-test.3 - Local-First native UI base */
var HuangDouUIV190=(function(){
  var V='1.9.1-test.3',ROOT='hiker://files/rules/asset-core-local/huangdou-test/assets/';
  function asset(n){var p=ROOT+String(n)+'.svg';try{return fileExist(p)?getPath(p):p;}catch(e){return p;}}
  var DESIGN={brand:'#F2BC3B',active:'#FFF3D4',muted:'#7C8494',icons:{library:asset('library'),topic:asset('topic'),mine:asset('mine'),settings:asset('settings')}};
  ...
  function card(c,x,col){x=x||{};var poster=c.imgUrl(x.img||''),meta=[];if(x.ep)meta.push(x.ep);if(x.heat)meta.push('热度 '+c.compactHeat(x.heat));if(x.tags&&x.tags.length)meta.push(c.maskText(x.tags[0]));return{title:c.maskText(x.title||'未命名'),desc:meta.join(' · '),img:poster,pic_url:poster,url:page('hddjDetail',{hddj_url:x.url||'',hddj_title:x.title||'',hddj_cover:poster}),col_type:col||getItem('hddj_col_v190','movie_3'),extra:{lineVisible:false,hddj_url:x.url||'',hddj_title:x.title||'',hddj_cover:poster}};}
  function topicCard(c,x){x=x||{};var poster=c.imgUrl(x.img||'');return{title:c.maskText(x.title||'专题'),desc:[x.count,c.maskText(x.desc||'')].filter(Boolean).join(' · '),img:poster,pic_url:poster,url:page('hddjTopic',{hddj_topic_url:x.url||'',hddj_title:x.title||''}),col_type:'movie_2',extra:{lineVisible:false,hddj_topic_url:x.url||'',hddj_title:x.title||''}};}
