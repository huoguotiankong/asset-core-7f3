/* 黄豆短剧 Detail / Topic / Settings 1.9.0-test.2 */
var HuangDouDetailV190=(function(){
  ...
  function safeHttp(c,raw){
    raw=String(raw||'').trim();if(!raw)return '';
    if(/^https?:\/\//i.test(raw))return raw;
    if(raw.charAt(0)==='/')return c.abs(raw);
    return '';
  }
  ...
  function detail(c,p){
    ...
    if(!url){setPageTitle(hint||'详情加载失败');setResult([U.empty('详情地址无效','请返回列表重新进入；Test2 已禁止使用保留参数 url 传递详情地址')]);return;}
    try{
  ...
  function settings(c,p){
    ...
    d.push({title:'版本 1.9.0-test.2',desc:'修复二级页保留参数 url 冲突；Native UI / PlaybackAdapter / 播放诊断 / 手动线路自检',url:'hiker://empty',col_type:'long_text'});setResult(d);
  }
  return{version:'1.9.0-test.2',detail:detail,topic:topic,settings:settings};
})();
