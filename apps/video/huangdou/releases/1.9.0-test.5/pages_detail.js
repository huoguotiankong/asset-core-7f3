/* 黄豆短剧 Detail / Topic / Settings 1.9.0-test.5 */
var HuangDouDetailV190=(function(){
  ...
  function detail(c,p){
    ...
    try{
      ...
      if(target)d.push({title:(targetEp&&targetEp.locked)?('🔒 第 '+target+' 集 · 付费/解锁'):(last>0?'继续第 '+last+' 集':'立即播放'),url:$('#noLoading#').lazyRule(function(id,ep,locked){return $.require('hddj').play(id,ep,locked);},x.id,target,!!(targetEp&&targetEp.locked)),col_type:'text_center_1',extra:{backgroundColor:D.active,lineVisible:false}});
      ...
      if(x.eps.length){
        ...
        d.push({title:'选集',desc:'共 '+x.eps.length+' 集 · '+(reverse?'倒序':'正序')+'（点击切换） · 🔒 为官网付费/需授权内容',url:$('#noLoading#').lazyRule(function(){setItem('hddj_reverse_v190',getItem('hddj_reverse_v190','0')==='1'?'0':'1');refreshPage(false);return'hiker://empty';}),col_type:'text_1',extra:{lineVisible:false}});
        ...
        show.forEach(function(ep){
          var current=last===ep.no,label='第'+ep.no+'集'+(ep.locked?' 🔒':'');
  ...
  function settings(c,p){
    ...
    d.push({title:'版本 1.9.0-test.5',desc:'付费/需授权剧集统一显示 🔒；播放协议继续沿用 Test4 会话保持 + Token + HLS 预检，不绕过官网权限',url:'hiker://empty',col_type:'long_text'});setResult(d);
  }
  return{version:'1.9.0-test.5',detail:detail,topic:topic,settings:settings};
})();
