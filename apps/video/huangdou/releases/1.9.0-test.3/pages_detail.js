/* 黄豆短剧 Detail / Topic / Settings 1.9.0-test.3 */
var HuangDouDetailV190=(function(){
  ...
  function detail(c,p){
    ...
    try{
      ...
      var last=parseInt(getItem('hddj_last_'+x.id,'0')||'0'),target=last>0?last:(x.eps.length?x.eps[0].no:0),fav=c.isFav(x.url);
      if(target)d.push({title:last>0?'继续第 '+last+' 集':'立即播放',url:$('#noLoading#').lazyRule(function(id,ep){return $.require('hddj').play(id,ep);},x.id,target),col_type:'text_center_1',extra:{backgroundColor:D.active,lineVisible:false}});
      ...
      if(x.eps.length){
        ...
        show.forEach(function(ep){
          var current=last===ep.no,label='第'+ep.no+'集';
          d.push({title:label,url:$('#noLoading#').lazyRule(function(id,no){return $.require('hddj').play(id,no);},x.id,ep.no),col_type:'text_4',extra:{backgroundColor:current?D.active:'',lineVisible:false}});
        });
      }
  ...
  function settings(c,p){
    ...
    d.push(U.line());d.push(U.section(c,'播放','单线路直连优先'));
    var st=p.strategy(),name=st==='web'?'网页兼容':'稳定直连（推荐）';
    d.push({title:'播放策略',desc:name,url:$('#noLoading#').lazyRule(function(){var ops=['稳定直连（推荐）','网页兼容'];var now=getItem('hddj_play_strategy_v3','direct'),idx=now==='web'?1:0;return'select://'+JSON.stringify({title:'播放策略',options:ops,selectedIndex:idx,col:1,js:$.toString(function(){setItem('hddj_play_strategy_v3',input==='网页兼容'?'web':'direct');refreshPage(false);})});}),col_type:'text_1'});
    ...
    d.push({title:'恢复 1.9 默认设置',url:$('#noLoading#').lazyRule(function(){clearItem('hddj_col_v190');clearItem('hddj_ep_group_v190');clearItem('hddj_reverse_v190');clearItem('hddj_play_strategy_v2');clearItem('hddj_play_strategy_v3');clearMyVar('hddj_home_tab_v190');clearMyVar('hddj_library_cat_v190');clearMyVar('hddj_mine_tab_v190');refreshPage(false);return'toast://已恢复默认设置';}),col_type:'text_center_1'});
    d.push({title:'版本 1.9.0-test.3',desc:'详情操作降噪 / 选集排序移出网格 / 所有剧集先尝试 Token / 单线路直连 / 取消无效 webRule fallback',url:'hiker://empty',col_type:'long_text'});setResult(d);
  }
  return{version:'1.9.0-test.3',detail:detail,topic:topic,settings:settings};
})();
