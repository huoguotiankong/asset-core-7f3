/* 黄果短剧 0.1.0-test.1 Core / Endpoint / Storage */
var HuangGuoCoreV1=(function(){
  ...
  function businessHtml(s){s=String(s||'');return s.length>500&&(s.indexOf('hg-drama')>=0||s.indexOf('recommend')>=0||s.indexOf('黄果')>=0||s.indexOf('短剧')>=0);}
  function candidates(html){var out=[],seen={};function add(u){var o=origin(u);if(!o||seen[o])return;seen[o]=1;out.push(o);}var s=String(html||''),re=/线路\s*\d+[\s\S]{0,800}?([a-zA-Z0-9][-a-zA-Z0-9]*\.[a-zA-Z0-9][-a-zA-Z0-9.]*\.[a-zA-Z]{2,})/g,m;while((m=re.exec(s))!==null)add('https://'+m[1]);var re2=/https?:\\?\/\\?\/([a-zA-Z0-9][-a-zA-Z0-9.]*\.[a-zA-Z]{2,})(?:[\\\/"'\s]|$)/g;while((m=re2.exec(s))!==null)add('https://'+m[1].replace(/\\/g,''));return out;}
  ...
  function discover(){var all=[],seen={},i,j,h,cs;function add(o){o=origin(o);if(o&&!seen[o]){seen[o]=1;all.push(o);}}for(i=0;i<DISCOVERY.length;i++){h=fetchText(DISCOVERY[i],{timeout:7000});cs=candidates(h);for(j=0;j<cs.length;j++)add(cs[j]);}for(i=0;i<all.length;i++){if(validate(all[i])){storeEndpoint(all[i]);diag('ENDPOINT_OK',all[i],'discovery');return all[i];}}diag('ENDPOINT_FAIL','discovery','no-valid-origin');return'';}
