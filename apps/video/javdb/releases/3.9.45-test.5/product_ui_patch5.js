...
(function(J){
  ...
  J.filters=function(){
    ...
    if(page===1){
      ...
      br5(d);
      for(i=0;i<tagGroups.length;i++){g=tagGroups[i]||{};cid=String(g.category_id||'');d.push(tab5(i===0?'标签':simp(J.mask(g.category||cid)),group===cid,$('#noLoading#').lazyRule(function(v){putMyVar('jdb3_cat46_group',v);refreshPage(false);return'hiker://empty';},cid)));}
      br5(d);
