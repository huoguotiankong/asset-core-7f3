...
(function(J){
  ...
  J.filters=function(){
    ...
    if(page===1){
      ...
      br5(d);
      d.push(tab5('标签',false,'hiker://empty'));
      for(i=0;i<tagGroups.length;i++){g=tagGroups[i]||{};cid=String(g.category_id||'');d.push(tab5(simp(J.mask(g.category||cid)),group===cid,$('#noLoading#').lazyRule(function(v){putMyVar('jdb3_cat46_group',v);refreshPage(false);return'hiker://empty';},cid)));}
      br5(d);
      ...
      if(current){
        ...
        d.push(tab5(simp(J.mask(current.category||cid)),false,$('#noLoading#').lazyRule(function(ids){var a=getMyVar('jdb3_cat46_extra','').split(',').filter(Boolean),rm=String(ids||'').split(','),n=[],i;for(i=0;i<a.length;i++)if(rm.indexOf(a[i])<0)n.push(a[i]);putMyVar('jdb3_cat46_extra',n.join(','));refreshPage(false);return'hiker://empty';},ids.join(','))));
