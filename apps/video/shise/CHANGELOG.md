...
状态：**第三轮实机反馈后的定向修复；已发布 Test6，等待正片播放 / 标签 / 人物 / 分类复测；无 Stable。**

### Test5 实机结果
- [实机确认] Test5 详情页可以正常进入，但仍没有识别出详情标签。
- [实机确认] Test5 纯视频页没有得到可信正片直链，网页嗅探实际命中了一个约 16 秒的广告视频；因此“第一个 m3u8 = 正片”的假设在当前 `shise.me` 已失效。
- [实机确认] Test5 人物页从当前 `/videos.html` 的 `model-container` 汇总仍为空，说明当前视色页面的人物字段和同站型旧阅读源继续存在差异。
- [实机反馈] 分类页面仍未达到可用状态，因此不能继续把小黄书旧 `series-id` 表直接视为视色当前真实分类合同。

### Test6 修复
- 不再直接使用第一个 m3u8。新播放解析会同时扫描 `main-container`、整页和最多 4 个 iframe 播放页，建立媒体候选集合。
- 对前 5 个候选实际请求 m3u8 清单并统计 `#EXTINF` 总时长；已知时长 `<=45s` 的候选强降权，针对本次实机命中的 16 秒广告流；数分钟以上候选显著升权。
- 对上下文中出现 `69lover / advert / preroll / banner / promo / vast / ads` 等广告特征的候选额外降权。
- 主播放只交付排序后的可信候选；点击播放时优先调用海阔官方 `clearM3u8Ad` 继续处理清单内广告片段。若没有可信候选，不再自动调用 `video://`，避免再次把广告误当正片。
- 新增“播放诊断”页，显示候选来源（main/page/iframe）、探测时长与评分，便于下一轮实机直接判断正片候选是否已经出现。
- 标签解析不再只认 `.contentTag`：增加 `.content-tag / .tags / .tag-list / .labels / .keywords`，并扫描 `/videos/tag-* / series-* / category-* / genre-* / label-*` 链接，最后以 `meta keywords` 兜底。
- 人物解析增加 `/model(s)/ / star(s)/ / actor(s)/ / actress / performer(s)/` 链接识别；如果当前页面只有人物文本而无独立详情链接，则人物页自动转为“按人物名搜索作品”，避免继续显示完全空白页面。
- 分类页不再直接套用同站型阅读源历史 `series-id` 表，改为实时读取当前 `shise.me/videos.html` 中存在的实际 `/videos/...` 分类链接，并自动转为 `{page}` 分页模板。
- Test5 `core.js + pages.js` 保持不可变；Test6 以独立 `runtime_patch.js` 定向覆盖本轮实机失败点，Release 仍通过 `main` 加载，无 `hiker-cloud` 正式依赖。

### Test6 重点回归
1. 覆盖导入 Test6，设置页确认 `Test 0.1.0-test.6 · Build 10106`。
2. 进入刚才会播放 16 秒广告的同一视频详情：如果识别到候选，立即播放按钮应显示“探测时长约 N 秒”；N 不应再是约 16 秒。
3. 如果仍无法直接播放，进入“播放诊断”，截图候选 1-3 的“时长 / 来源 / score”，不要再使用网页嗅探判断正片。
4. 详情页检查是否出现标签区；如果仍为空，需以当前页面的真实 HTML/DOM 继续收窄字段。
5. 人物页应至少出现“真实人物链接”或“人物名搜索”其中一种结果，不应继续只显示空白。
6. 分类页应以当前站点实际分类链接生成；任选一个分类向下滚动至少 2-3 页确认 `fypage` 有效。
7. 正片播放、人物、标签、分类均未通过前不得晋级 Stable。

## 0.1.0-test.5 / Build 10105 — 2026-09-24

状态：**第二轮实机反馈后的站点契约重建；已发布 Test5，等待播放 / 分类分页 / 人物 / 标签复测；无 Stable。**

### Test4 实机结果
- [实机确认] Test4 已修复详情 URL 无协议问题，视频详情页可以正常进入，标题、封面和简介能够显示。
- [实机确认] Test4 虽显示“1 条播放线路”，但海阔播放器为 `0 kb/s / 00:00`，实际播放失败。
- [实机确认] 详情页没有显示标签。
- [实机确认] 人物页请求 `https://shise.me/models.html` 成功，但 `.item.model` 为空，说明 `/models.html + .item.model` 并不是当前视色的人物列表合同。
- [实机确认] 分类内容页只能出现 12 个视频，不能继续向下取得后续分类分页。

### 重新核对同站型阅读源得到的真实合同
- [阅读源确认] 视频发现列表使用 `class.item video`，视频卡人物信息位于 `.model-container@a@text`。
- [阅读源确认] 纯视频正文只读取 `class.main-container@all`，从该容器中提取被引号包裹的 `.m3u8`；并没有把纯视频页的 `var videos/mp4` 当主合同。
- [阅读源确认] 视频详情标签字段使用 `.contentTag@text`。
- [阅读源确认] 视频分类不是从首页导航临时猜测，而是明确的 `/videos/series-<id>/<page>.html` 路由；阅读源已经维护了中文内容、日本内容、现场内容、电影内容、其他内容等完整分类组。
- [实现修正] 海阔二级可分页列表入口显式加入 `page=fypage`；Test4 的二级分类 URL 没有携带该分页模式，因此实机始终只有首屏 12 项。

### Test5 修复
- Runtime 拆分为 `core.js + pages.js` 两个模块，避免继续把所有站点合同堆在单文件页面逻辑里。
- 播放链改为：`main-container` 精确提取原始 m3u8 → **原始直链优先** → Header 兼容备用 → `video://` 网页嗅探兜底；不再让 Test4 的泛媒体解析和 Header 拼接成为默认主线路。
- 分类页使用阅读源明确的 `series-xxx/{page}.html` 分类合同；分类入口全部使用 `page=fypage`，后续滚动应请求真实第 2、3 页。
- 人物页不再访问空的 `/models.html`，改为从每一页 `.item.video` 的 `model-container` 链接实时汇总人物，并随 `fypage` 继续向下扩展。
- 人物详情直接打开 `model-container` 中实际 href，并从该页面解析 `.item.video` 关联作品。
- 视频详情新增 `.contentTag` 标签区；存在可用 `/videos/...` 标签链接时可直接进入对应分页列表。
- 安装 Shell 继续沿用 Test3 已实机验证可导入的中性命名、Unicode 标题、空图标方案。

### Test5 重点回归
1. 覆盖导入 Test5，设置页确认 `Test 0.1.0-test.5 · Build 10105`。
2. 进入刚才同一个视频详情，先点“立即播放”；若仍失败，再进入“播放线路与兼容模式”分别测试“原始直链 / Header 兼容 / 网页嗅探”。
3. 详情页应出现“标签”区；若仍为空，需要继续按该具体详情 HTML 调整 `contentTag` 容器解析。
4. 分类页应出现完整分类组；任选一个分类连续向下滚动至少 2-3 页，确认不再停在 12 项。
5. 人物页应从当前视频分页汇总人物；继续下滑应加载下一页视频中的人物，而不是请求 `/models.html`。
6. 人物详情应展示关联 `.item.video` 作品。
