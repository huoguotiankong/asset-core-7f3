/* 汤头条 0.1.0-test.6 Native UI */
var TangTouTiaoUIV015=(function(){
  ...
  function videoCard(c,x,col){var meta=[];if(x.author)meta.push(x.author);if(x.duration)meta.push(x.duration);if(x.plays)meta.push(x.plays);var p=c.routeParams?c.routeParams(x):{id:x.id,title:x.title,author:x.author};return{title:x.title||'未命名',desc:meta.join(' · '),img:x.cover||'',pic_url:x.cover||'',url:c.page('tttDetail',p),col_type:col||'movie_2',extra:{lineVisible:false,ttt_id:x.id}};}
  function comicCard(c,x){return{title:x.title||'未命名',desc:x.desc||'',img:x.cover||'',pic_url:x.cover||'',url:'toast://漫画详情链将在下一轮接入',col_type:'movie_3',extra:{lineVisible:false,ttt_comic_id:x.id}};}
  function category(t,u,on){return{title:(on?'● ':'')+String(t||''),url:u||'hiker://empty',col_type:'text_3',extra:{lineVisible:false}};}
  ...
  return{design:D,empty:empty,section:section,line:line,blank:blank,chip:chip,videoCard:videoCard,comicCard:comicCard,category:category,icon:icon,state:state,error:error};
})();
