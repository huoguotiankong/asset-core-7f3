/* 汤头条 0.1.0-test.6 PlaybackAdapter / source_240 default + encrypted HLS proxy */
var TangTouTiaoPlaybackV015=(function(){
  var V='0.1.0-test.6';
  ...
  function makeProxy(){
    var code=$.toString(function(){
      ...
      try{
        ...
        var dur=0,mm=plain.match(/#EXTINF:([0-9.]+)/g)||[];for(var di=0;di<mm.length;di++){var dm=mm[di].match(/([0-9.]+)/);if(dm)dur+=Number(dm[1]||0);}setItem('ttt_last_play_diag',JSON.stringify({time:Date.now(),ok:true,mode:mode,dekey:!!dekey,refer:!!refer,xauth:!!xauth,remote:String(remote).substring(0,220),rawLength:trim.length,plainLength:plain.length,head:plain.substring(0,24),duration:Math.round(dur*10)/10,suspiciousShort:dur>0&&dur<4}));
        return lines.join('\n');
  ...
  function prepared(payload){var x=typeof payload==='string'?JSON.parse(payload):payload||{},src=Array.isArray(x.sources)?x.sources:[];if(!src.length)return{x:x,src:[],proxy:''};try{if(typeof TangTouTiaoProtocolV015==='object')TangTouTiaoProtocolV015.bootstrapSession(false);}catch(e){}return{x:x,src:src,proxy:makeProxy()};}
  ...
  function playBest(payload){var p=prepared(payload);if(!p.src.length)return'toast://没有可用播放线路';var idx=p.src.length-1,s=p.src[idx],u=proxyUrl(p.proxy,s.url,idx);setItem('ttt_last_play_sources',JSON.stringify({time:Date.now(),mode:'single-default-app',name:s.name,url:compactUrl(s.url)}));return u;}
  function playQuality(payload,name){var p=prepared(payload);if(!p.src.length)return'toast://没有可用播放线路';name=String(name||'');var idx=p.src.length-1;for(var i=0;i<p.src.length;i++)if(String(p.src[i].name)===name){idx=i;break;}var s=p.src[idx],u=proxyUrl(p.proxy,s.url,idx);setItem('ttt_last_play_sources',JSON.stringify({time:Date.now(),mode:'single-quality',name:s.name,url:compactUrl(s.url)}));return u;}
