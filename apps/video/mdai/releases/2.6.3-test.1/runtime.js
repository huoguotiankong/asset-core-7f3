/* 麻豆AI remote test runtime loader 2.6.3-test.1
...
var MDAIRemoteRuntime=(function(){
  ...
  var CACHE='mdai_remote_snapshot_263_test1';
  var BUILD='2.6.3-test.1';
  var instance=null;
  ...
  function source(){
    ...
    if(!valid(s))throw new Error('麻豆AI远程业务快照格式错误');
  ...
  function module(){
    ...
    for(var i=0;i<pages.length;i++)if(String(pages[i].path||'')==='mdai'){p=pages[i];break;}
    if(!p||!p.rule)throw new Error('麻豆AI远程业务模块缺失：mdai');
    var old=$.exports;
    eval(String(p.rule));
    var m=$.exports;
    $.exports=old;
    if(!m||typeof m!=='object')throw new Error('麻豆AI远程业务模块导出失败');
    instance=m;
    return instance;
  }
