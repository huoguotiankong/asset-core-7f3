/* ACFun Test Bootstrap v6.6.0 - alpha10 recovery from Alpha8 baseline */
...
ACFUN_BOOTSTRAP_VERSION='6.6.0-test';
ACFUN_BOOTSTRAP_URL='https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_test_v066.js?v=6600';
ACFUN_HC_CONFIG={
 id:'acfun-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',latestPath:'apps/video/acfun/test.json',moduleHeaders:{'Cache-Control':'no-cache'},minBuild:161,
 defaultRelease:{schema:1,id:'acfun-test',name:'ACFun',version:'0.6.0-alpha10',build:161,ref:'main',modules:[
 ...
 ],verify:{global:'ac',property:'build',equals:'2026.08.22-v0.6.0-alpha10'}}
};

ACFunBoot.legacyLoaderCode=function(){return "require('"+ACFUN_BOOTSTRAP_URL+"',{headers:{'Cache-Control':'no-cache'}},6600);ACFunBoot.loadOnly();";};
ACFunBoot.ensureBaseline=function(m){var info=m.info(ACFUN_HC_CONFIG),cur=info.current||{};if(Number(cur.build||0)<161){var r=m.resetToDefault(ACFUN_HC_CONFIG);if(!r||!r.ok)throw new Error('ACFun测试版迁移0.6.0-alpha10失败：'+((r&&r.error)||'unknown'));setItem('acfun_test_bootstrap_migrated','0.6.0-alpha10');}};
...
ACFunBoot.updatePage=function(){
 ...
 d.push({title:'当前测试版本 '+cur.version,desc:'Build '+cur.build+' · Shell '+ACFUN_BOOTSTRAP_VERSION+'\nAlpha9 已因实机出现封面、播放、分类、小说/有声与漫画多域回归而冻结。Alpha10 不继承 Alpha9 业务模块，直接从最后一版关键能力较完整的 Alpha8 模块链恢复，再只叠加封面字段兼容、视频播放刷新、小说/有声分类与 txt/音频章节恢复、社区详情合并。',col_type:'long_text',url:'hiker://empty'});
