/* ACFun Next Test Bootstrap v9.0.0 - alpha11 credential recovery + signed-host repair + comic border crop */
var ACFUN_NEXT_BOOTSTRAP_VERSION='9.0.0-test';
var ACFUN_NEXT_BOOTSTRAP_URL='https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_test_v090.js?v=9000';
...
var ACFUN_NEXT_CONFIG={
 id:'acfun-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',latestPath:'apps/video/acfun/test.json',moduleHeaders:{'Cache-Control':'no-cache'},minBuild:10011,
 defaultRelease:{schema:1,id:'acfun-test',name:'ACFun',version:'1.0.0-alpha11',build:10011,ref:'main',modules:[
...
var ACFunNextBoot={
 ...
 run:function(action){this.loadOnly();switch(String(action||'home')){case'home':return ACFunNext.home();case'search':return ACFunNext.search();case'searchCenter':return ACFunNext.searchCenter();case'station':return ACFunNext.stationPage();case'detail':return ACFunNext.detail();case'comicReader':return ACFunNext.comicReader();case'fictionReader':return ACFunNext.fictionReader();case'comments':return ACFunNext.comments();case'favorites':return ACFunNext.localPage('fav');case'history':return ACFunNext.localPage('hist');case'mine':return ACFunNext.mine();case'settings':return ACFunNext.settings();case'diag':return ACFunNext.diag();default:throw new Error('未知 ACFun Next 动作: '+action);}},
 ...
 updatePage:function(){var d=[];setPageTitle('ACFun 版本与恢复');var info;try{info=this.info();}catch(e){setResult([{title:'版本信息加载失败',desc:String(e.message||e),col_type:'long_text',url:'hiker://empty'}]);return;}var cur=info.current||ACFUN_NEXT_CONFIG.defaultRelease,pre=info.previous||null;d.push({title:'当前测试版 '+cur.version,desc:'Build '+cur.build+' · Shell '+ACFUN_NEXT_BOOTSTRAP_VERSION+'\nAlpha11 不再继续重复 Header/播放器试错：补齐 APK 中独立 playback credential 来源，探测 sys/getDynamicDomain / sys/sdk-config / 带 videoId/path 的 player referer，并优先验证“保留现有 auth_key 只切换 CDN 域名”的签名线路修复；漫画所有正文页统一裁掉四边源图白色画布后再全宽显示。Stable 0.4.9 保持不变。',col_type:'long_text',url:'hiker://empty'});d.push({title:'检查测试通道更新',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){showLoading('检查中…');try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.check();hideLoading();refreshPage(false);return'toast://'+(r.hasUpdate?('发现 '+r.latest.version):'已是当前测试版')}catch(e){hideLoading();return'toast://检查失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,9000)});d.push({title:'重新加载当前测试版',desc:'清当前版本模块缓存后重新拉取，不清收藏、历史和设置。',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){showLoading('重新加载…');try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.reinstall();hideLoading();return'toast://'+(r.ok?'已重新加载':'失败：'+r.error)}catch(e){hideLoading();return'toast://失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,9000)});d.push({title:'回退上一测试版本',desc:pre?(pre.version+' / Build '+pre.build):'暂无上一测试版本',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.rollback();if(r.ok){refreshPage(false);return'toast://已回退 '+r.current.version}return'toast://'+r.error}catch(e){return'toast://回退失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,9000)});d.push({title:'恢复 Alpha11 基线',desc:'强制回到本壳内置 Build10011，不清用户数据。',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.resetDefault();if(r.ok){refreshPage(false);return'toast://已恢复 '+r.current.version}return'toast://'+r.error}catch(e){return'toast://恢复失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,9000)});d.push({title:'恢复正式版 Stable 0.4.9',desc:'到“我的规则仓库 → ACFun → 正式版”覆盖导入。',col_type:'long_text',url:'hiker://home@我的规则仓库||hiker://home'});setResult(d);}
};
