/* ACFun Next Test Bootstrap v8.0.0 - independent clean rewrite chain */
var ACFUN_NEXT_BOOTSTRAP_VERSION='8.0.0-test';
var ACFUN_NEXT_BOOTSTRAP_URL='https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_test_v080.js?v=8000';
...
var ACFUN_NEXT_CONFIG={
 id:'acfun-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',latestPath:'apps/video/acfun/test.json',moduleHeaders:{'Cache-Control':'no-cache'},minBuild:10001,
 defaultRelease:{schema:1,id:'acfun-test',name:'ACFun',version:'1.0.0-alpha1',build:10001,ref:'main',modules:[
 ...
 ],verify:{global:'ACFunNext',property:'build',equals:'2026.08.23-v1.0.0-alpha1'}}
};
var ACFunNextBoot={
 ...
 updatePage:function(){
  var d=[];setPageTitle('ACFun 版本与恢复');var info;try{info=this.info();}catch(e){setResult([{title:'版本信息加载失败',desc:String(e.message||e),col_type:'long_text',url:'hiker://empty'}]);return;}
  var cur=info.current||ACFUN_NEXT_CONFIG.defaultRelease,pre=info.previous||null;
  d.push({title:'当前测试版 '+cur.version,desc:'Build '+cur.build+' · Shell '+ACFUN_NEXT_BOOTSTRAP_VERSION+'\n这是全新独立链，不再加载 0.4.9/0.6.0 的历史 UI 补丁栈。Stable 0.4.9 保持不变。',col_type:'long_text',url:'hiker://empty'});
  d.push({title:'检查测试通道更新',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){showLoading('检查中…');try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.check();hideLoading();refreshPage(false);return'toast://'+(r.hasUpdate?('发现 '+r.latest.version):'已是当前测试版')}catch(e){hideLoading();return'toast://检查失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,8000)});
  d.push({title:'重新加载当前测试版',desc:'清当前版本模块缓存后重新拉取，不清收藏、历史和设置。',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){showLoading('重新加载…');try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.reinstall();hideLoading();return'toast://'+(r.ok?'已重新加载':'失败：'+r.error)}catch(e){hideLoading();return'toast://失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,8000)});
  d.push({title:'回退上一测试版本',desc:pre?(pre.version+' / Build '+pre.build):'暂无上一测试版本',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.rollback();if(r.ok){refreshPage(false);return'toast://已回退 '+r.current.version}return'toast://'+r.error}catch(e){return'toast://回退失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,8000)});
  d.push({title:'恢复 Clean Alpha1 基线',desc:'强制回到本壳内置 Build10001，不清用户数据。',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(boot,ver){try{require(boot,{headers:{'Cache-Control':'no-cache'}},ver);var r=ACFunNextBoot.resetDefault();if(r.ok){refreshPage(false);return'toast://已恢复 '+r.current.version}return'toast://'+r.error}catch(e){return'toast://恢复失败：'+String(e.message||e)}},ACFUN_NEXT_BOOTSTRAP_URL,8000)});
  d.push({title:'恢复正式版 Stable 0.4.9',desc:'到“我的规则仓库 → ACFun → 正式版”覆盖导入。正式版没有被本次重写改动。',col_type:'long_text',url:'hiker://home@我的规则仓库||hiker://home'});
  setResult(d);
 }
};
