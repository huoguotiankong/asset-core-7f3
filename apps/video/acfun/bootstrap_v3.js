/* ACFun Remote Bootstrap v3.0.1 */
var ACFUN_BOOTSTRAP_VERSION='3.0.1';
var ACFUN_BOOTSTRAP_URL='https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_v3.js?v=3';
...
var ACFUN_HC_CONFIG={
 id:'acfun',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',latestPath:'apps/video/acfun/latest.json',moduleHeaders:{'Cache-Control':'no-cache'},
 defaultRelease:{schema:1,id:'acfun',name:'ACFun',version:'0.2.3',build:123,ref:'main',modules:[
  {name:'core',path:'acfun_core_v018.js'},
  {name:'protocol',path:'acfun_patch_v019.js'},
  {name:'diagnostics',path:'acfun_diag_v019.js'},
  {name:'ui-object-cover',path:'acfun_ui_v022.js'},
  {name:'ui-final-image',path:'acfun_ui_v023.js'}
 ],verify:{global:'ac',property:'build',equals:'2026.08.20-v0.2.3'}}
};
var ACFunBoot={
 requireManager:function(){require(ACFUN_MANAGER_URL,{headers:{'Cache-Control':'no-cache'}},200);if(typeof HikerCloudRemote!=='object')throw new Error('远程模块管理器加载失败');return HikerCloudRemote;},
 legacyLoaderCode:function(){return "require('"+ACFUN_BOOTSTRAP_URL+"',{headers:{'Cache-Control':'no-cache'}},300);ACFunBoot.loadOnly();";},
 ...
 ensureBaseline:function(m){var info=m.info(ACFUN_HC_CONFIG),cur=info.current||{};if(Number(cur.build||0)<123){var r=m.resetToDefault(ACFUN_HC_CONFIG);if(!r||!r.ok)throw new Error('ACFun迁移0.2.3失败：'+((r&&r.error)||'unknown'));setItem('acfun_bootstrap_migrated','0.2.3');}},
 ...
 updatePage:function(){
  ...
  d.push({title:latest?('云端版本 '+latest.version+(lc.hasUpdate?' ↑ 可更新':' ✓ 已是最新')):'检查云端版本',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(){showLoading('检查更新…');try{require('https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_v3.js?v=3',{headers:{'Cache-Control':'no-cache'}},300);var r=ACFunBoot.check();hideLoading();refreshPage(false);return 'toast://'+(r.hasUpdate?('发现 '+r.latest.version):'已是最新版');}catch(e){hideLoading();return 'toast://检查失败：'+(e.message||e);}})});
  d.push({title:'立即更新',desc:'加载并校验新模块后再切换版本。',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(){showLoading('更新中…');require('https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_v3.js?v=3',{headers:{'Cache-Control':'no-cache'}},300);var r=ACFunBoot.update();hideLoading();if(r.ok){clearItem('hc_acfun_last_check');refreshPage(false);return 'toast://'+(r.changed?('已更新到 '+r.current.version):'已经是最新版');}return 'toast://更新失败：'+r.error;})});
  d.push({title:'重新加载当前版本',desc:'清除当前模块缓存并重新拉取，不清除收藏历史。',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(){showLoading('重新加载…');require('https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_v3.js?v=3',{headers:{'Cache-Control':'no-cache'}},300);var r=ACFunBoot.reinstall();hideLoading();return 'toast://'+(r.ok?'已重新缓存':'失败：'+r.error);})});
  d.push({title:'回退上一版本',desc:pre?('上一版本：'+pre.version):'没有上一版本记录',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(){require('https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_v3.js?v=3',{headers:{'Cache-Control':'no-cache'}},300);var r=ACFunBoot.rollback();if(r.ok){refreshPage(false);return 'toast://已回退 '+r.current.version;}return 'toast://'+r.error;})});
  d.push({title:'恢复稳定基线 0.2.3',desc:'恢复 Build 123，不删除收藏、历史和接口登录态。',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(){require('https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_v3.js?v=3',{headers:{'Cache-Control':'no-cache'}},300);var r=ACFunBoot.resetDefault();if(r.ok){refreshPage(false);return 'toast://已恢复 '+r.current.version;}return 'toast://失败：'+r.error;})});
  setResult(d);
 }
};