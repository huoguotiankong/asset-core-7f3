/* ACFun Test Bootstrap v6.4.0 - alpha8 same-page filter/native reader baseline */
...
ACFUN_BOOTSTRAP_VERSION='6.4.0-test';
ACFUN_BOOTSTRAP_URL='https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfun/bootstrap_test_v064.js?v=6400';
ACFUN_HC_CONFIG={
 id:'acfun-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',latestPath:'apps/video/acfun/test.json',moduleHeaders:{'Cache-Control':'no-cache'},minBuild:159,
 defaultRelease:{schema:1,id:'acfun-test',name:'ACFun',version:'0.6.0-alpha8',build:159,ref:'main',modules:[
 ...
 ],verify:{global:'ac',property:'build',equals:'2026.08.22-v0.6.0-alpha8'}}
};

ACFunBoot.legacyLoaderCode=function(){return "require('"+ACFUN_BOOTSTRAP_URL+"',{headers:{'Cache-Control':'no-cache'}},6400);ACFunBoot.loadOnly();";};
ACFunBoot.ensureBaseline=function(m){var info=m.info(ACFUN_HC_CONFIG),cur=info.current||{};if(Number(cur.build||0)<159){var r=m.resetToDefault(ACFUN_HC_CONFIG);if(!r||!r.ok)throw new Error('ACFun测试版迁移0.6.0-alpha8失败：'+((r&&r.error)||'unknown'));setItem('acfun_test_bootstrap_migrated','0.6.0-alpha8');}};
...
ACFunBoot.updatePage=function(){
 ...
 d.push({title:'当前测试版本 '+cur.version,desc:'Build '+cur.build+' · Shell '+ACFUN_BOOTSTRAP_VERSION+'\nAlpha8 把筛选改回首页弹层即时刷新；社区/小说/有声与主栏目全部同页切换，减少页面堆叠；漫画章节改用海阔 pics:// 原生多图阅读；精选/里番改用独立严格参数；小说/有声新增 GET/POST 双请求矩阵。',col_type:'long_text',url:'hiker://empty'});
