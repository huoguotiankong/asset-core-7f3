/* 911爆料 0.1.1-test.1 Local-First entry */
var Bl911Local=(function(){
...
var BREF='7cd2a345eedb03cd153e2d6cfa9226f29dd31c22';
var BPATH='apps/video/911baoliao/releases/0.1.1-test.1/local_bundle_builder.js';
...
function ensure(){if(fileExist(BUILDER))return BUILDER;var us=['https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/'+BREF+'/'+BPATH,'https://cdn.jsdelivr.net/gh/huoguotiankong/asset-core-7f3@'+BREF+'/'+BPATH,'https://github.com/huoguotiankong/asset-core-7f3/raw/'+BREF+'/'+BPATH],es=[];for(var i=0;i<us.length;i++){try{var s=String(fetch(us[i],{timeout:6500,headers:{'Cache-Control':'public, max-age=31536000, immutable'}})||'');if(bad(s)||s.indexOf(VERSION)<0||s.indexOf('strict-marker+bootstrap-assignment+asset-root-v3')<0)throw new Error('无效响应');writeFile(BUILDER,s);if(!fileExist(BUILDER))throw new Error('写入失败');return BUILDER;}catch(e){es.push((i+1)+':'+String(e.message||e));}}throw new Error('911爆料本地 Builder 下载失败：'+es.join(' | '));}
function req(p){var u=getPath(p),r=null;try{r=require(u);}catch(e0){try{deleteCache(u);}catch(e1){}r=require(u);}return r;}
function builder(){ensure();req(BUILDER);if(typeof Bl911LocalBuilder!=='object'||typeof Bl911LocalBuilder.load!=='function')throw new Error('Bl911LocalBuilder 未导出');return Bl911LocalBuilder;}
function load(){var x=builder().load(),r=x.runtime;if(!r||typeof r.module!=='function'||String(r.localFirstVersion||'')!==VERSION||Number(r.localFirstBuild||0)!==BUILD)throw new Error('911爆料 Local-First runtime 未就绪');return{meta:x.meta,runtime:r};}
