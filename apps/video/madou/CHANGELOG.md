...
### 实机结果
- Test9 已把 Test8 的多跳 player 请求从详情首屏移走，但用户继续确认：**进入影片详情仍偏慢，同组分类切换更慢，甚至长时间加载不出来**。
- 这说明瓶颈不只在播放解析：Test9 仍要求每个详情首屏先请求完整详情 HTML；分类切换仍在 `R.list()` 内重新读取首页并重建分类树，再请求目标分类。
- 同时 Test9 Release 已叠到 9 个历史模块/补丁，继续追加第 10 个“修一点”的覆盖层只会让热路径和运行事实更难控制。

### Test10 性能重构
- **不继续继承 Test3–Test9 补丁栈。** 活动 Release 直接收敛为 3 个模块：
  - Test1 Core；
  - Test1 Runtime；
  - Test10 `performance_runtime.js`。
- Test10 在最后一个性能运行时中一次性恢复并统一：中文规则路由、卡片伪资源过滤、14 大分类/373 小分类层级、本地收藏/历史配额保护、同级分类原页切换、详情 UI 与播放兜底。
- 这样热点二级页不再每次依次加载/覆盖 Test2、Test3、Test5、Test6、Test7、Test8、Test9 多层逻辑。

### 分类热路径
- 首次从首页解析成功后，把分类树只保存为小型结构化 `CategoryModel` JSON，不保存 HTML。
- “全部分类”和分类内容页只读取这个索引；同级小分类切换**不再请求首页、不再重建分类树**。
- 最近 6 个分类页保存最多 30 张压缩后的 `CardModel`，TTL 20 分钟；回切最近分类直接本地渲染。
- 第一次进入未缓存分类只允许一次目标分类请求；普通热路径取消 `fetch → request` 双 transport 连续兜底，单次网络预算约 6.5 秒。
- 请求失败但有旧结构化卡片时优先显示 stale 内容，不让页面长期空白等待。

### Detail Seed First
- 首页/分类/搜索/收藏/历史的影片路由现在同时携带 `u + title + raw cover + desc`。
- 正常从这些入口进入详情时，首屏直接使用已有 Seed 渲染 Hero、标题、摘要、播放和收藏，**默认零网络请求**。
- 标签和相关推荐从首屏移出，改成明确的“加载标签与相关推荐”按需动作；只有用户主动展开时才请求完整详情。
- 老收藏/外部直达等没有 Seed 的入口仍允许一次详情请求恢复元数据。
- 播放协议研究继续与详情首屏隔离；未获得纯 API 直链前不再让 player/iframe 研究拖慢详情打开。

### 存储与稳定性
- 继续在详情/分类热路径精确清理历史 `madou_v1_ / madou_v2_` raw HTML key。
- 历史/收藏收紧为更小的字段和总 JSON 预算；分类索引与 Feed 缓存也都有硬上限，避免再次接近海阔 1MB 私有存储上限。
- 不缓存完整 HTML，只缓存结构化分类、最近卡片、历史和收藏。

### 发布门禁
- `performance_runtime.js` 已本地执行 `node --check` 通过。
- `bootstrap_test_v10_b10110.js` 已执行 `node --check` 通过。
- `release.json` 已完成 JSON 解析检查；Test10 Shell 内嵌规则 JSON 也已重新解析通过。
- Test10 使用新 Release / Bootstrap / Shell，`minBuild=10110`，不原地覆盖 Test9。
- `test.json / channels.json / app manifest / registry.json / root manifest.json / manifest_meta.json` 已切到 Test10；云仓 revision 为 `202608231616`，itemCount 12。
- 新增跨程序事故文档：`docs/INCIDENT_HOT_PATH_PATCH_STACK_AND_SEED_FIRST_DETAIL_20260823.md`，记录 Seed-First Detail、分类索引缓存、补丁栈收敛和热路径一次请求预算。

### Test10 回归重点
1. 从首页/分类列表点击影片，详情首屏应接近即时出现；如果仍明显慢，优先排查 Remote Shell/模块加载，而不是再加详情请求。
2. 进入一个大分类后连续切换多个同组小分类：第一次进入未缓存分类最多等待一次目标请求；回切最近访问分类应直接命中卡片缓存。
3. “全部分类”切换 14 个大类应只做本地状态刷新，不应出现网络长等待。
4. 只有点击“加载标签与相关推荐”时才允许详情网络请求。
5. 播放仍单独观察；纯 API 免嗅尚未完成，不把浏览器兜底冒充直连完成。
...
- UI 会直接提示当前播放路径：
  ...
  - `兼容解析 · 当前页未命中免嗅直链` = 仍走 `video://`，下一轮需要针对真实 player HTML/接口继续收紧。
