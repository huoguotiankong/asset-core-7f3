...
- Test：`2.0.0-test.26` / Build `20026`。
...
- Stable 尚未晋级。
- Test26 直接基于 **Test24 已实机验证的头像 + Shell/Bootstrap 运行链** 与 Test25 单请求楼中楼性能链，按既定路线进入创作者中心与产品体验阶段；不改播放解析、登录 Cookie、漫画和基础评论正文协议。
...
用户确认头像已经恢复后，头像诊断完成使命，Test25 从设置页移除临时“头像诊断”区域，保留真实运行版本、重载、兼容 Recovery、检查更新、更新和回退能力。
...
按既定路线在头像和回复性能之后进入“作者目录 + 独立作者主页”阶段。本版不新增 Shell 页面，继续复用已经存在的 `hanimeVideoResults` 路由，通过 `creatorType` 参数切换作者目录、作者主页和上传者主页，避免为了一个页面再次升级 Shell。

### 作者目录
直接复刻 Han1mePlus 当前作者搜索契约：
...
- 筛选页增加“作者目录 / 综合搜索”入口。
- 搜索页增加“查找同名作者”入口，不在普通影片搜索首屏额外并发请求作者接口。
- 作者目录支持关键词、头像、作品数文本与分页。
- 作者元数据增加 10 分钟短缓存，只缓存公开名称/头像/canonical query/count，不保存账号或 Cookie 数据。

### 独立作者主页
- 从视频详情作者卡、作者目录卡进入独立作者视图。
- Header 显示作者头像、作者身份与官网返回作品数信息。
- 作者作品继续使用 canonical query 调用现有影片搜索链，保持卡片、播放和分页一致。
- 提供“在全部影片中搜索 / 作者目录”两个辅助入口。

### 上传者主页
- 继续使用 Test17 已实机通过的 `/user/<id>` 公共上传链。
- UI 与作者主页统一：头像/身份 Header → 官网主页/同名搜索 → 公开上传作品 → 分页。
- 作者和上传者仍严格分开，不把上传账号误当作品作者。
...
- “加入片单”直接复用现有 `hanimePlaylistPicker`；未登录先进入登录页。
- 选集继续保持横向 `scroll_button`，当前集显示 `▶`。
- “作者与上传者”改为“创作者”，两个头像卡分别进入作者主页/上传者主页。
- 作品信息、标签、简介、画质、相关推荐继续保留，减少技术说明抢首屏。

### 搜索 / 筛选 / 设置
- 搜索：影片结果和作者发现分流，避免一个输入触发两套联网数据。
- 筛选：最上方先给“作者目录 / 综合搜索”，再进入类型、排序、日期、时长和标签。
- 设置：账号 → 界面 → 网络 → 测试版本 → 维护，移除重复的运行版本块；重载/回退/Recovery 收到“维护”，正常用户优先看到常用功能。
- 更新相关 `lazyRule` 继续强制内部 `require(bootstrap_test_v4.js)`，不得复活 `HanimeBoot 未定义` 事故。
...
当前 Test23/24/25/26 使用海阔内置 `xpathArray/xpa` 表达上述契约；Test24 已实机证明真实头像可显示。
...
## Test26 待实机回归
- [ ] 设置页显示 `2.0.0-test.26 · Build 20026 · Shell v4`，且没有头像诊断。
- [ ] Test25 的作者/主评论/回复真实头像保持正常。
- [ ] “查看 X 条回复”仍保持单请求优化，回复正文/时间/头像对应不退化。
- [ ] 详情页四个主操作一行显示合理：播放 / 评论 / 加入片单 / 下载原片。
- [ ] 作者卡点击进入作者主页；头像、作者名、作品列表和分页正常。
- [ ] 上传者卡点击进入公开上传主页；真实 `/user/<id>` 作品链不退化。
- [ ] 作者目录能打开；输入作者名后能出现匹配作者卡。
- [ ] 搜索页“查找同名作者”和筛选页“作者目录”路径正常。
- [ ] 加入片单：已登录进入 picker，未登录进入登录页。
- [ ] 设置页常用功能层级比 Test25 更简洁；维护操作仍可用。
- [ ] 登录、播放、真选集、片库、漫画无回归。

## 后续顺序
1. 实机闭环 Test26 作者目录/作者主页/详情 UI，并按截图继续做纯 UI 微调。
2. 下一阶段：评论点赞/点踩、点赞数、举报等官网元信息与交互。
3. 再做账号中心/订阅作者增强，并把订阅状态与作者主页联动。
4. 主要功能稳定后做 Consolidated Candidate，压缩 Test15～26 的历史增量链。

---
## 版本记录
### 2.0.0-test.26 / Build 20026 / 2026-08-22
- 新增作者目录与作者搜索卡，按 Han1mePlus `.search-artist-card` 契约解析。
- 新增独立作者主页和统一上传者主页体验，复用现有 `hanimeVideoResults` 路由，不升级 Shell。
- 详情首屏主操作重排为播放 / 评论 / 加入片单 / 下载原片。
- 搜索/筛选增加作者发现入口；设置页按账号/界面/网络/版本/维护重新分层。
- 保留 Test25 单请求楼中楼、45 秒线程缓存和 Test24 实机通过的头像链。
