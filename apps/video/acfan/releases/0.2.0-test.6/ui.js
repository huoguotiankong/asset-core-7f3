/* ACFAN 0.2.0-test.6 native design system */
var ACFAN6UI=(function(){
  var C=ACFAN6Core,I=ACFAN6Image,ROOT='https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/main/apps/video/acfan/assets/v2/';
  ...
  function detailUrl(e){return C.page('acfanT6Detail',{acf_kind:e.kind||'video',acf_id:e.id||'',acf_title:e.title||'',acf_img:e.img||'',acf_author:e.author||'',acf_desc:e.desc||''});}
  ...
  function card(e,opt){e=e||{};opt=opt||{};var img=I.url(e.img||''),kind=e.kind||'video',col=opt.col||((kind==='video'||kind==='short')?'movie_2':((kind==='community'||kind==='ai')?(img?'movie_1_left_pic':'text_1'):'movie_3'));return{title:C.clean(e.title||'未命名'),desc:metadata(e),img:img,pic_url:img,url:detailUrl(e),col_type:col,extra:{lineVisible:false,acf_kind:kind,acf_id:e.id||''}};}
  function hero(e){e=e||{};var img=I.url(e.img||'');return{title:C.clean(e.title||'未命名'),desc:[e.author,metadata(e),e.desc].filter(Boolean).join(' · '),img:img,pic_url:img,url:'hiker://empty',col_type:'movie_1_vertical_pic_blur',extra:{lineVisible:false}};}
