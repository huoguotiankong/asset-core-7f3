/* 夜社短剧 Runtime/UI 0.1.0-test.4 */
var YesheRemoteRuntime=(function(){
  var VERSION='0.1.0-test.4',BUILD=10104,P=YesheProtocol,D=YesheProvider,PB=YeshePlayback;
  var BOOT='https://cdn.jsdelivr.net/gh/huoguotiankong/asset-core-7f3@main/apps/video/yeshe/bootstrap_test_v4_b10104.js?v=10104';
  ...
  function lazyPlay(url){return $('').lazyRule(function(boot,u){require(boot,{headers:{'Cache-Control':'no-cache'}},10104);return YesheBoot.module().play(u);},BOOT,url);}
  function lazyFavorite(item){return $('').lazyRule(function(boot,j){require(boot,{headers:{'Cache-Control':'no-cache'}},10104);var m=YesheBoot.module(),o={};try{o=JSON.parse(j);}catch(e){}return m.toggleFavorite(o);},BOOT,JSON.stringify(item||{}));}
  function lazyTab(key,val){return $('').lazyRule(function(k,v){putMyVar(k,v);refreshPage(false);return'hiker://empty';},key,val);}
  function card(x,cls){
    x=x||{};return{title:clean(x.title)||'未命名内容',desc:clean(x.desc)||'点击查看详情',pic_url:image(x.cover,x.url),url:route('yesheDetail',{yeshe_url:x.url,yeshe_title:x.title}),col_type:'movie_3',extra:{id:'yeshe-card-'+hash(x.url),cls:cls||'yeshe-cards'}};
  }
  ...
  function settings(){
    ...
    d.push({title:'当前线路',desc:host,col_type:'text_1',url:$('').lazyRule(function(boot){require(boot,{headers:{'Cache-Control':'no-cache'}},10104);var h=YesheBoot.module().refreshHost();refreshPage(false);return'toast://当前线路：'+h;},BOOT)});
