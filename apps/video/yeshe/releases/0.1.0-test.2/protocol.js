...
var YesheProtocol=(function(){
  ...
  function firstImage(raw,base){
    var re=/<img\b([^>]*)>/ig,m,u='';while((m=re.exec(s(raw)))){u=attr(m[1],'data-original')||attr(m[1],'data-src')||attr(m[1],'data-lazy-src')||attr(m[1],'data-echo')||attr(m[1],'src');u=abs(u,base);if(u&&!/(logo|favicon|icon[-_.]|avatar|loading|placeholder|banner|ads?\b|qrcode)/i.test(u))return u;}return'';
  }
