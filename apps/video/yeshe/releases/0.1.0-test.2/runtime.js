/* 夜社短剧 Runtime/UI 0.1.0-test.2 */
var YesheRemoteRuntime=(function(){
  var VERSION='0.1.0-test.2',BUILD=10102,P=YesheProtocol,D=YesheProvider,PB=YeshePlayback;
  var BOOT='https://cdn.jsdelivr.net/gh/huoguotiankong/asset-core-7f3@main/apps/video/yeshe/bootstrap_test_v2_b10102.js?v=10102';
  ...
  function route(path,params){var u='hiker://page/'+path+'?rule='+encodeURIComponent(ruleTitle())+'&simple=true',k;params=params||{};for(k in params)if(params[k]!=null&&s(params[k])!=='')u+='&'+encodeURIComponent(k)+'='+encodeURIComponent(s(params[k]));return u;}
  ...
  function lazyPlay(url){return $('').lazyRule(function(boot,u){require(boot,{headers:{'Cache-Control':'no-cache'}},10102);return YesheBoot.module().play(u);},BOOT,url);}
  function lazyFavorite(item){return $('').lazyRule(function(boot,j){require(boot,{headers:{'Cache-Control':'no-cache'}},10102);var m=YesheBoot.module(),o={};try{o=JSON.parse(j);}catch(e){}return m.toggleFavorite(o);},BOOT,JSON.stringify(item||{}));}
  ...
  function category(){
    var name=param('yeshe_category_name')||'分类',url=param('yeshe_category_url')||'',page=D.pageNo(),r=D.category(name,url,page),d=[];
  ...
  function detailSeed(){return{title:param('yeshe_title')||'',cover:param('yeshe_cover')||'',desc:param('yeshe_desc')||''};}
  function detail(){
    var url=param('yeshe_url')||'',o=D.detail(url,detailSeed()),d=[],i,lineMap={},lines=[],ln,key='yeshe_line_'+hash(url),sel='',ordKey='yeshe_order_'+hash(url),order=getMyVar(ordKey,'asc'),eps=[],chapters=o.chapters||[];
  ...
  function settings(){
    ...
    d.push({title:'当前线路',desc:host,col_type:'text_1',url:$('').lazyRule(function(boot){require(boot,{headers:{'Cache-Control':'no-cache'}},10102);var h=YesheBoot.module().refreshHost();refreshPage(false);return'toast://当前线路：'+h;},BOOT)});
