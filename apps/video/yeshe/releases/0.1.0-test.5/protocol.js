...
var YesheProtocol=(function(){
  ...
  function nav(html,base){
    var as=anchors(html,base),map={},all=[],i,a,n,re=/(\/type\/|\/filter\/|\/list\/|\/category\/|\/class\/|[?&](?:type|tid|cat|category|class)=)/i;
    for(i=0;i<as.length;i++){
      a=as[i];n=clean(a.text||a.title);
      if(!n||n.length>24||!re.test(a.href))continue;
      if(!map[n])map[n]=a.href;
      all.push({name:n,url:a.href});
    }
    var er=/<[^>]+(?:data-href|data-url)\s*=\s*["']([^"']+)["'][^>]*>([\s\S]*?)<\/[^>]+>/ig,m,u,t,seen={};
    while((m=er.exec(s(html)))&&all.length<500){
      u=abs(m[1],base);t=clean(m[2]);
      if(!u||!t||t.length>24||!re.test(u)||seen[u])continue;
      seen[u]=1;if(!map[t])map[t]=u;all.push({name:t,url:u});
    }
    return{map:map,all:all};
  }
  function navCached(force){
    var now=new Date().getTime(),old=null,base=discover(false),urls=[base+'/',base+'/type/13.html?chl=yeshetv',base+'/filter/13.html?chl=yeshetv'],map={},all=[],seen={},i,r,n,j,x,transport=[],total=0;
    ...
    for(i=0;i<urls.length;i++){
      r=requestUrl(urls[i],{timeout:9000});
      if(!looksHtml(r.body))continue;
      transport.push(r.transport||'direct');total+=s(r.body).length;n=nav(r.body,urls[i]);
      for(j=0;j<n.all.length;j++){x=n.all[j];if(!map[x.name])map[x.name]=x.url;if(!seen[x.url]){seen[x.url]=1;all.push(x);}}
    }
    var obj={map:map,all:all,host:base,time:now,transport:transport.join('+'),len:total};
    try{setItem(NAV_KEY,JSON.stringify(obj));}catch(e2){}
    saveDiag({ok:all.length>0,stage:'NAV',host:base,transport:obj.transport,len:total,count:all.length});
    return obj;
  }
  ...
  function episodes(html,base,current){
    var as=anchors(html,base),out=[],seen={},i,a,m,id='',cur=s(current).match(/\/play\/(\d+)\/(\d+)\/(\d+)\.html(?:[?#]|$)/i);
    if(cur)id=cur[1];
    for(i=0;i<as.length;i++){
      a=as[i];m=a.href.match(/\/play\/(\d+)\/(\d+)\/(\d+)\.html(?:[?#]|$)/i);
      if(!m)continue;if(id&&m[1]!==id)continue;
      var line=Number(m[2])||1,ep=Number(m[3])||1,key=m[1]+'|'+line+'|'+ep;
      if(seen[key])continue;seen[key]=1;
      out.push({id:m[1],line:line,episode:ep,title:'第'+ep+'集',url:a.href});
    }
    out.sort(function(x,y){if(x.line!==y.line)return x.line-y.line;return x.episode-y.episode;});
    return out;
  }
