/* 夜社短剧 Runtime/UI 0.1.0-test.6 */
var YesheRemoteRuntime=(function(){
  var VERSION='0.1.0-test.6',BUILD=10106,P=YesheProtocol,D=YesheProvider,PB=YeshePlayback;
  var BOOT='https://cdn.jsdelivr.net/gh/huoguotiankong/asset-core-7f3@main/apps/video/yeshe/bootstrap_test_v6_b10106.js?v=10106';
  var ASSET='https://cdn.jsdelivr.net/gh/huoguotiankong/asset-core-7f3@main/apps/video/yeshe/assets/';
  var FAV='yeshe_favorites_v1',HIS='yeshe_history_v1',SH='yeshe_search_history_v1',TAB='yeshe_home_tab_v1',DETAIL_SEED='yeshe_detail_seed_v2',READER_PREFIX='yeshe_reader_seed_v1_';
  ...
  function lazyPlay(url){return $('').lazyRule(function(boot,u){require(boot,{headers:{'Cache-Control':'no-cache'}},10106);return YesheBoot.module().play(u);},BOOT,url);}
  function lazyFavorite(item){return $('').lazyRule(function(boot,j){require(boot,{headers:{'Cache-Control':'no-cache'}},10106);var m=YesheBoot.module(),o={};try{o=JSON.parse(j);}catch(e){}return m.toggleFavorite(o);},BOOT,JSON.stringify(item||{}));}
  ...
  function detail(){
    ...
    }else if((o.kind==='comic'||o.kind==='gallery')&&o.gallery.length){
      d.push({title:'开始阅读 · '+o.gallery.length+' 张',desc:'独立纵向阅读页 · 返回一次回到详情',col_type:'text_center_1',url:openReader({title:o.title,ref:o.url,images:o.gallery}),extra:{backgroundColor:'#E93650',lineVisible:false}});
  ...
  function openReader(o){
    o=o||{};var key=READER_PREFIX+hash(s(o.ref)||s(o.title));
    return $('').lazyRule(function(k,j){
      try{setItem(k,j);}catch(e){}
      return'hiker://page/yesheReader?rule=&simple=true&yeshe_reader_key='+encodeURIComponent(k);
    },key,JSON.stringify(o));
  }
  function reader(){
    var key=param('yeshe_reader_key')||'',o={};try{o=JSON.parse(getItem(key,'{}')||'{}');}catch(e){o={};}
    var imgs=o.images||[],ref=s(o.ref),d=[],i;
    pageTitle(clean(o.title)||'漫画阅读');
    if(!imgs.length){setResult([{title:'图片读取失败',desc:'没有恢复到本章节图片，可返回详情重试。',col_type:'text_1',url:'hiker://empty'}]);return;}
    for(i=0;i<imgs.length;i++)d.push({title:'',desc:'',pic_url:image(imgs[i],ref),url:'hiker://empty',col_type:'pic_1_full',extra:{id:'yeshe-reader-'+i,lineVisible:false}});
    setResult(d);
  }
  ...
  function settings(){
    ...
    d.push({title:'当前线路',desc:host,col_type:'text_1',url:$('').lazyRule(function(boot){require(boot,{headers:{'Cache-Control':'no-cache'}},10106);var h=YesheBoot.module().refreshHost();refreshPage(false);return'toast://当前线路：'+h;},BOOT)});
  ...
  function module(){return{version:VERSION,build:BUILD,home:home,catalog:catalog,category:category,search:search,detail:detail,reader:reader,favorites:favorites,history:history,mine:mine,settings:settings,play:play,toggleFavorite:toggleFavorite,refreshHost:refreshHost,route:routePage};}
