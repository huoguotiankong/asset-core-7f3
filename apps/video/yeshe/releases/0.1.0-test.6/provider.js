/* 夜社短剧 Provider 0.1.0-test.6 */
var YesheProvider=(function(){
  var P=YesheProtocol,VERSION='0.1.0-test.6',BUILD=10106;
  ...
  function detailKind(groupId){
    if(groupId==='comic')return'comic';
    if(groupId==='novel')return'novel';
    return'video';
  }
  function catalog(){
    ...
    for(i=0;i<GROUPS.length;i++){
      ...
      for(j=0;j<GROUPS[i].items.length;j++){
        ...
        g.items.push({name:x,url:url,kind:detailKind(GROUPS[i].id),group:GROUPS[i].id,available:!!url});
        if(url)used[url]=1;
      }
      groups.push(g);
    }
    for(i=0;i<(n.all||[]).length;i++){
      ...
      extras.push({name:x.name,url:x.url,kind:'unknown',group:'dynamic',available:true});used[x.url]=1;
    }
  ...
  function detail(url,seed){
    ...
    if(kindPlayable(kind)||/\/play\/\d+\/\d+\/\d+\.html/i.test(url)){
      ...
      related=relatedCards(html,url,'video');
    }else if(kind==='comic'){
      gallery=P.gallery(html,url);
      if(cover){var cg=[];for(var gi=0;gi<gallery.length;gi++)if(gallery[gi]!==cover)cg.push(gallery[gi]);if(cg.length>=3)gallery=cg;}
      related=relatedCards(html,url,'comic');
    }else if(kind==='novel'){
