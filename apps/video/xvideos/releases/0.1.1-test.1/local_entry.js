/* XVideos 0.1.1-test.1 Local-First entry */
var XVideosLocal=(function(){
...
var BREF='8d871c1d85b209cc2da64cab01d8905c2e12a81b';
var BPATH='apps/video/xvideos/releases/0.1.1-test.1/local_bundle_builder.js';
...
function ensure(){if(fileExist(BUILDER))return BUILDER;var us=['https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/'+BREF+'/'+BPATH,'https://cdn.jsdelivr.net/gh/huoguotiankong/asset-core-7f3@'+BREF+'/'+BPATH,'https://github.com/huoguotiankong/asset-core-7f3/raw/'+BREF+'/'+BPATH],es=[];for(var i=0;i<us.length;i++){try{var s=String(fetch(us[i],{timeout:7000,headers:{'Cache-Control':'public, max-age=31536000, immutable'}})||'');if(bad(s)||s.indexOf(VERSION)<0||s.indexOf('strict-marker+bootstrap-expression+asset-root+app-root-v4')<0)throw new Error('无效响应');writeFile(BUILDER,s);if(!fileExist(BUILDER))throw new Error('写入失败');return BUILDER;}catch(e){es.push((i+1)+':'+String(e.message||e));}}throw new Error('XVideos 本地 Builder 下载失败：'+es.join(' | '));}
function req(p){var u=getPath(p),r=null;try{r=require(u);}catch(e0){try{deleteCache(u);}catch(e1){}r=require(u);}return r;}
function builder(){ensure();req(BUILDER);if(typeof XVideosLocalBuilder!=='object'||typeof XVideosLocalBuilder.load!=='function')throw new Error('XVideosLocalBuilder 未导出');return XVideosLocalBuilder;}
function load(){var x=builder().load(),r=x.runtime;if(!r||typeof r.module!=='function'||String(r.localFirstVersion||'')!==VERSION||Number(r.localFirstBuild||0)!==BUILD)throw new Error('XVideos Local-First runtime 未就绪');return{meta:x.meta,runtime:r};}
