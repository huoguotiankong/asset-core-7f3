/* xChina Test11 post-pages UX/playback fixes */
(function(K,R){
  if(!K||!R)throw new Error('Test11 postpatch: Runtime 未加载');
  ...
  function tagRoute(tag){
    ...
    if(tag.kind==='model'||/\/models?\//i.test(p))return page('xchinaModel',{xc_url:h,t:'model'});
  ...
  R.media=function(){
    ...
    if(!xs.length){d.push(empty('没有解析到正片媒体地址','严格按上传阅读源的 main-container 规则取视频；网页嗅探已关闭，避免再次命中广告。'));d.push(btn('🌐 原站核对','web://'+url));}
      ...
      for(i=0;i<xs.length;i++){
        u=xs[i];
        d.push({title:'▶️ 推荐播放 '+(i+1),desc:/m3u8/i.test(u)?'HLS：先带 Host=s2.playhls.com 预检并 cacheM3u8；失败再普通 Header':'MP4：Referer / Origin / UA',url:K.playMedia(u,url),col_type:'text_1',extra:{lineVisible:false,id:url+'#t11'+i}});
  ...
  R.settings=function(){
    ...
    d.push(section('ℹ️ 状态','Test 0.1.0-test.11 · Build 10111'));
    d.push(section('📝 本轮','女优页前置 6 部作品；标签只保留 model-container + 阅读源指定 tags 位置；封面重新按卡片 style 精确取图；HLS 增加旧站已知 Host=s2.playhls.com + cacheM3u8 交付。'));
    setResult(d);
  };
  K.version='0.1.0-test.11';K.build=10111;R.version='0.1.0-test.11';R.build=10111;
