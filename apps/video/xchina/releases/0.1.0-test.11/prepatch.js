/* xChina Test11 pre-pages contracts
 * Runs after Test9 core and before Test9 pages. No dependency on Test10 patches.
 */
(function(K){
  if(!K||String(K.version)!=='0.1.0-test.9'||Number(K.build)!==10109)throw new Error('Test11 prepatch: Test9 Core 未加载');
  var C=K.C,s=K.s,trim=K.trim,decode=K.decode,strip=K.strip,abs=K.abs,origin=K.origin;
  C.baseKey='xc_t11_base';C.lastBaseKey='xc_t11_last_base';C.cachePrefix='xc_t11_html_';
  ...
  function coverFor(u,listUrl,type){
    ...
    var ref=(type==='comic'&&/litu100\.xyz/i.test(listUrl))?origin(listUrl)+'/':C.primary+'/',h={'User-Agent':C.ua,'Referer':ref,'Accept-Language':'zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7'};
    /* Do not attach unrelated site cookies to CDN image hosts. */
  ...
  function tagAnchorFromBlock(block,url){
    var a=K.domArray(block,'body&&a'),i,n,h;if(!a||!a.length)a=K.domArray(block,'a');
    for(i=0;i<a.length;i++){n=strip(K.domHtml(a[i],'a&&Text')||K.domHtml(a[i],'Text')||a[i]);h=K.domUrl(a[i],'a&&href',url);if(n)return{name:n,href:h};}
    return null;
  }
  ...
  function detailTags(html,url,type){
    ...
    var out=[],seen={},models=K.domArray(html,'.model-container&&a'),i,n,h,divs,idx=[4,1,3],b,t;
    for(i=0;i<models.length;i++){n=cleanTagText(K.domHtml(models[i],'a&&Text')||K.domHtml(models[i],'Text')||models[i]);h=K.domUrl(models[i],'a&&href',url);if(!badTag(n)&&!seen[n]){seen[n]=1;out.push({name:n,href:h,kind:'model'});}}
    divs=K.domArray(html,'.tags&&div');
    for(i=0;i<idx.length;i++)if(divs&&divs.length>idx[i]){
      b=divs[idx[i]];t=cleanTagText(K.domHtml(b,'div&&Text')||K.domHtml(b,'Text')||b);if(!badTag(t)&&!seen[t]){h='';var ta=tagAnchorFromBlock(b,url);if(ta&&cleanTagText(ta.name)===t)h=ta.href;seen[t]=1;out.push({name:t,href:h,kind:'content'});}
    }
    return out.slice(0,6);
  }

  function modelAllUrl(html,url){
    var as=K.domArray(html,'body&&a'),i,n,h;for(i=0;i<as.length;i++){n=strip(K.domHtml(as[i],'a&&Text')||K.domHtml(as[i],'Text')||as[i]);if(/全部视频|全部作品/.test(n)){h=K.domUrl(as[i],'a&&href',url);if(h)return h;}}return'';
  }
  ...
  function playMedia(u,pageUrl){
    return $(u).lazyRule(function(media,ref,ua){
      ...
      var isHls=/\.m3u8(?:$|[?#])/i.test(String(media)),ck=cookie(ref)||cookie(media),base={'User-Agent':ua,'Referer':ref,'Origin':org(ref)},h1={},h2={},k,txt='';
      for(k in base){h1[k]=base[k];h2[k]=base[k];}if(ck){h1.Cookie=ck;h2.Cookie=ck;}h1.Host='s2.playhls.com';
      if(isHls){
        try{txt=String(fetch(media,{timeout:6500,headers:h1})||'');if(txt.indexOf('#EXTM3U')>=0){var c1=cacheM3u8(media,{headers:h1});if(c1)return c1+'#isVideo=true#';}}catch(e3){}
        try{txt=String(fetch(media,{timeout:6500,headers:h2})||'');if(txt.indexOf('#EXTM3U')>=0){var c2=cacheM3u8(media,{headers:h2});if(c2)return c2+'#isVideo=true#';}}catch(e4){}
        return media+';{'+hs(h1)+'}#isVideo=true#';
      }
      return media+';{'+hs(h2)+'}#isVideo=true#';
    },u,pageUrl,C.ua);
  }
  ...
  K.mediaFromHtml=mediaFromHtml;K.detailTags=detailTags;K.modelAllUrl=modelAllUrl;K.modelCount=modelCount;K.playMedia=playMedia;K.playPlainHeader=playPlainHeader;K._t11Main=balancedMain;
})(XChinaTest9Core);
