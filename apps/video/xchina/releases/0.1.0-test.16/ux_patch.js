...
(function(K,R){
  ...
  R.home=function(){var type=getMyVar('xc_home_type','photo'),p=K.currentPage(),r=K.listResult(type,K.listPath(type,p)),d=[];if(p===1){d.push({title:'小黄书 · 发现',desc:'小说 / 套图 / 漫画 / 视频',url:'hiker://empty',col_type:'text_1',extra:{lineVisible:false}});K.tabs(d,'xc_home_type',type);[['🔎 搜索','xchinaSearch'],['🗂 分类','xchinaCategories'],['👩 模特','xchinaCatalog'],['♥ 收藏','xchinaSettings']].forEach(function(x){d.push(K.btn(x[0],page(x[1],x[1]==='xchinaCatalog'?{t:'model',name:'模特'}:{})));});d.push(K.section(K.typeIcon(type)+' '+K.typeName(type)+' · 最新',''));}if(!r.items.length)d.push(K.empty('没有解析到内容',r.r.url));for(var i=0;i<r.items.length;i++)d.push(K.card(r.items[i]));setResult(d);};
  var oldSettings=R.settings;R.settings=function(){var f=favorites(),d=[],keys=Object.keys(f);d.push(K.section('♥ 已收藏模特',keys.length+' 位'));keys.forEach(function(u){d.push({title:'👩 '+f[u],url:page('xchinaModel',{xc_url:u,t:'model'}),col_type:'text_1'});});if(!keys.length)d.push(K.empty('暂无收藏','在模特详情页点击收藏'));setResult(d);};
