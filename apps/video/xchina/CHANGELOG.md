...
## 0.1.0-test.19 / Build 10119 — 2026-09-25

状态：**仅 Test；图片与头像等待实机验收。** 原网站公告视频服务器故障，播放问题暂缓。

- 套图、漫画卡片与阅读图片的 Referer 采用实际页面域名；保留源站/图片域/无 Header 三线路切换，视频卡片与播放路径不改。
- 模特头像按实际页面请求头加载，优先读取头像懒加载属性，必要时尝试 OG 图片；不将视频缩略图伪装成头像。
- 套图详情封面采用同一图片请求规则，清理简介后附带的推广文字与过长标签串。
- 已完成 JS 语法与定点契约检查；CDN 在当前运行环境被 Cloudflare 拒绝，图片显示必须由手机实机确认。Test18 可回退。


## 0.1.0-test.18 / Build 10118 — 2026-09-24

状态：**定位版 Test；无 Stable。播放、套图/漫画封面与模特头像仍未修复。**

- [实机确认] Test17 视频详情对应页面 `fetch` 成功，HTML 72971 字符，现有源码解析得到 M3U8 0 条。说明请求本身成功，但正片字段可能不在当前静态 `main-container` 契约；继续只改正则或 Header 缺乏依据。
- [实机确认] 模特页头像仍空白；套图首页封面仍成片空白。Test17 的三种图片线路尚未给出实机响应结果；当前无法区分错误 URL、403/防盗链、懒加载占位与解密。
- Test18 增加独立“运行诊断”页：全页/主容器 m3u8 次数、video/iframe/script 数量、前五个脚本或 iframe 地址特征；套图/漫画前两张卡片原始图片 URL；最近访问模特的头像候选 URL。查询值不展示 Cookie/Token，图片 URL 去掉 query 和 headers。
- 播放诊断增加“页面与图片诊断”入口。此轮不调整 Test17 的播放或图像业务代码，也不晋级 Stable。
- Test Shell `2026092412` / Build `10118`。下一步依据运行诊断一张截图或文本核对真实字段，再做定点修复。


## 0.1.0-test.17 / Build 10117 — 2026-09-24

状态：当前 Test；根据用户 Test16 实机截图返工；无 Stable。

### 实机证据与根因边界
- [实机确认] 视频点主播放出现“链接为空，规则有误！”；Test15 的 `webRule://页面@脚本` 最终返回空链接，未触达播放器。已撤销该主链，不能再报告播放成功。
- [实机确认] 套图首页大面积空白；套图图片页出现空白图片区域（并非“未解析到图片”提示），说明至少有图片候选但交付/请求失败。Test16 的属性优先扫描可能选到占位链接；尚缺具体图片响应码。
- [实机确认] 模特头像空白，简介混入“收录视频数”和作品标题。后者来自 Test13 的宽松个人简介提取。

### 本轮变更
- 视频详情先用 Test13 源码链提取 `main-container` 中的 M3U8；有候选才提供原生直连，没有则进入原网页 X5 播放器，并在播放诊断显示页面请求方式、HTML 长度和候选数。取消空链接 webRule 主链。原网页播放仍待实机确认，不等于原生正片可播。
- 图片正文优先按用户上传阅读源的原规则，在 `.photo-image/.comic-img-box` 容器中扫描绝对图片 URL；找不到才用 Test16 属性路径。阅读页加入源站 Referer、图片域 Referer、无 Header 三条手动对照，显示首图域名。
- 封面优先读取单卡 `.img@style` 的 CSS URL；模特头像尝试直接原图 URL，简介严格收敛已知默认语句。
- Test16 作为回退基线不原地覆盖；新增 Test17 repairPatch；Shell `2026092411`，Build `10117`。播放、图片、头像须实机验收后才能继续修或晋级 Stable。
