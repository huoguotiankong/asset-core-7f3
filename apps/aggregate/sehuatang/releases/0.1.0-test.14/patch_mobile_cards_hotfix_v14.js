/* 色花堂 0.1.0-test.14 / Build 10114 - Test13 hotfix: clean mobile card summaries + thread detail crash fix */
var SeHuaTangPatchTest14=(function(){
  var BASE=SeHuaTangRemoteRuntime;
  var VERSION='0.1.0-test.14',BUILD=10114,RULE_NAME='色花堂';
  ...
  function cleanSummary(ctx,title){
    var x=s(ctx),q=trim(title),t,m;
    x=x.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<img\b[^>]*>/gi,' ');
    x=x.replace(/\b(?:target|class|style|href|src|onclick|rel|id|data-[\w-]+)\s*=\s*(?:["'][^"']*["']|[^\s>]+)/gi,' ');
    t=strip(x);
    if(q){while(t.indexOf(q)>=0)t=t.replace(q,' ')}
    t=t.replace(/(?:查看帖子|查看全部|最后发表|最后回复|主题|回复|浏览|隐藏置顶帖|上一页|下一页|发表主题|社区版块)/g,' ');
    t=t.replace(/\b(?:target|blank|js-randomBg|js-appJump|admin)\b/gi,' ');
    t=t.replace(/(?:诚招代理|鲍鱼盒子最低\s*\d+\s*元)/g,' ');
    t=t.replace(/(?:\.\.\s*)?(?:\d+\s+){3,}\d+(?:\s+|$)/g,' ');
    t=t.replace(/\b\d{4}-\d{1,2}-\d{1,2}\b/g,' ');
    t=t.replace(/\s+/g,' ').trim();
    m=t.match(/((?:【[^】]{1,18}】\s*){1,3}[\s\S]{0,170})/);
    if(m&&m[1]&&m[1].length>25)t=trim(m[1]);
    if(t.length>170)t=t.slice(0,170)+'…';
    return t;
  }

  function parseMobileCards(html,base){
    var a=anchors(html,base),map={},order=[],out=[],freq={},i,x,id,t,sc,j,nextStart,ctx,imgs,c;
    for(i=0;i<a.length;i++){
      x=a[i];id=threadId(x.href);if(!id)continue;t=trim(x.text);sc=threadScore(t);
      if(!map[id]){map[id]={id:id,title:sc>-900?t:'',url:toMobile(x.href),score:sc,titleIndex:x.index}}
      else if(sc>map[id].score){map[id].title=t;map[id].score=sc;map[id].url=toMobile(x.href);map[id].titleIndex=x.index}
    }
    for(id in map)if(map.hasOwnProperty(id)&&!badThreadText(map[id].title))order.push(id);
    order.sort(function(a1,b1){return map[a1].titleIndex-map[b1].titleIndex});
    for(i=0;i<order.length;i++){
      id=order[i];x=map[id];nextStart=i+1<order.length?map[order[i+1]].titleIndex:s(html).length;
      if(nextStart<=x.titleIndex||nextStart-x.titleIndex>90000)nextStart=Math.min(s(html).length,x.titleIndex+65000);
      ctx=s(html).slice(x.titleIndex,nextStart);
      imgs=allImages(ctx,base,8);x.imgs=imgs;x.summary=cleanSummary(ctx,x.title);
      for(j=0;j<imgs.length;j++){c=canonical(imgs[j]);freq[c]=(freq[c]||0)+1}
      out.push(x);if(out.length>=100)break;
    }
    for(i=0;i<out.length;i++){
      imgs=[];for(j=0;j<(out[i].imgs||[]).length;j++){c=canonical(out[i].imgs[j]);if((freq[c]||0)>=3)continue;imgs.push(out[i].imgs[j]);if(imgs.length>=2)break}out[i].imgs=imgs;
    }
    return out;
  }
  ...
  function renderList(u){try{return s(fetchCodeByWebView(u,{headers:headers(false,u),timeout:25000,blockRules:['.woff','.woff2','.ttf'],checkJs:$.toString(function(ckKey,accessKey){
    ...
    if(age(body)){var ns=document.querySelectorAll('a,button,input[type="button"],input[type="submit"],[onclick],[role="button"],div');for(var i=0;i<ns.length;i++){if(age(txt(ns[i]))){try{ns[i].click();return null}catch(e){}}}return null}
    ...
    var imgs=document.querySelectorAll('img');for(var j=0;j<imgs.length;j++){var im=imgs[j],src=String(im.getAttribute('src')||'');if(!src||/none\.gif|loading|blank|placeholder|data:image/i.test(src)){var real=im.getAttribute('data-original')||im.getAttribute('data-src')||im.getAttribute('data-lazy-src')||im.getAttribute('file')||im.getAttribute('zoomfile')||'';if(real)try{im.setAttribute('src',real)}catch(e3){}}}
    window.__shtV14PreviewPass=(window.__shtV14PreviewPass||0)+1;
    if(window.__shtV14PreviewPass===1){try{window.scrollTo(0,Math.floor(document.body.scrollHeight*0.45))}catch(e4){}return null}
    if(window.__shtV14PreviewPass===2){try{window.scrollTo(0,document.body.scrollHeight)}catch(e5){}return null}
    try{window.scrollTo(0,0)}catch(e6){}return 'ready';
  ...
  function mergeMissing(primary,extra){var map={},i,j,x,seen;for(i=0;i<(extra||[]).length;i++)map[extra[i].id]=extra[i];for(i=0;i<(primary||[]).length;i++){x=map[primary[i].id];if(!x)continue;if(!primary[i].summary&&x.summary)primary[i].summary=x.summary;seen={};for(j=0;j<(primary[i].imgs||[]).length;j++)seen[canonical(primary[i].imgs[j])]=1;for(j=0;j<(x.imgs||[]).length&&primary[i].imgs.length<2;j++)if(!seen[canonical(x.imgs[j])]){seen[canonical(x.imgs[j])]=1;primary[i].imgs.push(x.imgs[j])}}return primary}
  function loadBoard(seed,pg){var mu=withPage(toMobile(seed),pg),pu=withPage(toPc(seed),pg),h,items,extra=[],diag=[];h=renderList(mu);items=parseMobileCards(h,mu);diag.push('m-web:'+h.length+'/'+items.length+'/'+previewCount(items)+'img');if(!items.length){h=fetchPage(mu,false);items=parseMobileCards(h,mu);diag.push('m-fetch:'+h.length+'/'+items.length+'/'+previewCount(items)+'img')}if(items.length&&previewCount(items)<Math.max(2,Math.floor(items.length/4))){h=fetchPage(pu,true);extra=parseMobileCards(h,pu);diag.push('pc-enrich:'+h.length+'/'+extra.length+'/'+previewCount(extra)+'img');items=mergeMissing(items,extra)}if(!items.length){h=fetchPage(pu,true);items=parseMobileCards(h,pu);diag.push('pc-fallback:'+h.length+'/'+items.length+'/'+previewCount(items)+'img')}return{items:items,url:mu,diag:diag.join('；')}}

  function pageKey(seed){var f=forumId(seed);return'sht_forum_page_v14_'+(f||s(seed).replace(/[^a-z0-9]/ig,'').slice(-24)||'x')}
  function pageButton(title,key,p,enabled){return{title:title,url:enabled?$('#noLoading#').lazyRule(function(k,v){putMyVar(k,String(v));refreshPage(false);return'hiker://empty';},key,p):'hiker://empty',col_type:'text_3',extra:{lineVisible:false,cls:'sht_v14_page'}}}
  function pageControls(key,pg,hasNext){return[pageButton(pg>1?'上一页':'第一页',key,Math.max(1,pg-1),pg>1),pageButton(pg>1?'回第1页':'第1页',key,1,pg>1),pageButton(hasNext?'下一页':'已到底',key,pg+1,hasNext)]}
  function renderCards(d,items,ref){var i,j,it,u,p,ct;for(i=0;i<items.length;i++){it=items[i];u=route('shtThread',{sht_url:toMobile(it.url),sht_name:it.title});d.push({title:it.title,desc:it.summary||'查看帖子',url:u,col_type:'text_1',extra:{lineVisible:false,cls:'sht_v14_card'}});ct=(it.imgs||[]).length===1?'pic_1_full':'pic_2';for(j=0;j<(it.imgs||[]).length&&j<2;j++){p=imageUrl(it.imgs[j],ref);d.push({title:'',desc:'',img:p,pic_url:p,url:u,col_type:ct,extra:{lineVisible:false,cls:'sht_v14_card'}})}d.push(line('sht_v14_card'))}}
  function forumBoard(){var d=[],seed=pageParam('sht_url',''),name=pageParam('sht_name','主题列表'),key=pageKey(seed),urlPg=Number(pageParam('sht_p','0')||0),pg,r,ctrl;if(urlPg>0){putMyVar(key,String(urlPg));pg=urlPg}else pg=Math.max(1,Number(getMyVar(key,'1')||1));setPageTitle(name);r=loadBoard(seed,pg);saveDiag('forum.mobile.v14','p='+pg+' · '+r.diag);if(pg===1){d.push(quick('手机版','x5://'+withPage(toMobile(seed),1),'web.svg'));d.push(quick('搜索',route('shtSearch'),'search.svg'));d.push(line());d.push(section(name,'手机端卡片 · 当前第 '+pg+' 页 · '+r.items.length+' 条主题 · '+previewCount(r.items)+' 张预览图'))}else d.push(section(name,'第 '+pg+' 页 · 页内刷新翻页'));ctrl=pageControls(key,pg,r.items.length>0);Array.prototype.push.apply(d,ctrl);d.push(line());renderCards(d,r.items,r.url);if(r.items.length){ctrl=pageControls(key,pg,true);Array.prototype.push.apply(d,ctrl)}else d.push(empty('本页没有解析到主题','可点上一页或回第1页'));setResult(d)}
  ...
  function emitImage(d,u,ref){var p=imageUrl(u,ref);d.push({title:'',desc:'',img:p,pic_url:p,url:'pics://'+p,col_type:'pic_1_full',extra:{lineVisible:false,imgLongClick:true}})}
  function orderedBody(d,body,ref){var x=s(body),re=/<img\b[^>]*>/gi,m,last=0,u,t,parts=0,seen={};while((m=re.exec(x))!==null){t=textBlock(x.slice(last,m.index));if(t){emitText(d,t);parts++}u=imgFromTag(m[0],ref);if(u&&!seen[canonical(u)]){seen[canonical(u)]=1;emitImage(d,u,ref);parts++}last=re.lastIndex}t=textBlock(x.slice(last));if(t){emitText(d,t);parts++}return parts}
