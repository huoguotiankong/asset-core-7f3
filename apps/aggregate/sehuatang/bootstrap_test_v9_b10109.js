/* 色花堂 Remote Test Bootstrap 0.1.0-test.9 - immutable overlay loader */
var SEHUATANG_BOOT_CONFIG={id:'sehuatang-test',branch:'main',repoRawRoot:'https://raw.githubusercontent.com/huoguotiankong/asset-core-7f3/',version:'0.1.0-test.9',build:10109,modules:[
...
var SeHuaTangBoot={loadOnly:function(){var root=SEHUATANG_BOOT_CONFIG.repoRawRoot+SEHUATANG_BOOT_CONFIG.branch+'/',loaded=[],i,m,url;for(i=0;i<SEHUATANG_BOOT_CONFIG.modules.length;i++){m=SEHUATANG_BOOT_CONFIG.modules[i];url=root+m.path+'?sehuatang_release=0.1.0-test.9';require(url,{headers:{'Cache-Control':'no-cache'}},m.ver);loaded.push({name:m.name,url:url});}if(typeof SeHuaTangRemoteRuntime==='undefined'||String(SeHuaTangRemoteRuntime.version)!=='0.1.0-test.9')throw new Error('色花堂 Test9 运行时校验失败');return{ok:true,release:{id:'sehuatang-test',version:'0.1.0-test.9',build:10109},loaded:loaded};},module:function(){this.loadOnly();return SeHuaTangRemoteRuntime.module();},info:function(){return{managerVersion:'direct-loader',current:{id:'sehuatang-test',version:'0.1.0-test.9',build:10109}};},check:function(){return{ok:true,current:{id:'sehuatang-test',version:'0.1.0-test.9',build:10109},hasUpdate:false};},update:function(){return{ok:false,changed:false,error:'测试版采用完整导入口令覆盖更新'};},rollback:function(){return{ok:false,error:'需要时覆盖导入 Test8'};},reinstall:function(){return this.loadOnly();}};
