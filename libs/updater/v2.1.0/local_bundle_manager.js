/* asset-core Local Bundle Manager v2.1.0
 * Local-first runtime installer: remote publish, local run.
 * - First install / version upgrade downloads immutable release modules into rule-private files.
 * - Active pointer switches only after every module is written and re-read successfully.
 * - Normal runtime can load private local files directly; GitHub/CDN is control-plane only.
 * - Previous local package is retained for fallback.
 */
var HikerLocalBundle=(function(){
  ...
  function writeText(n,s){saveFile(n,String(s),0);var back=String(readFile(n,0)||'');if(back!==String(s))throw new Error('本地写入校验失败: '+n);return true;}
  ...
  function templates(c){var a=c&&Array.isArray(c.repoTemplates)&&c.repoTemplates.length?c.repoTemplates:[
  ...
  function urls(c,path,ref){path=norm(path);ref=String(ref||'main');return templates(c).map(function(t){return String(t).replace(/\{ref\}/g,ref).replace(/\{path\}/g,path);});}
  ...
  function install(c,r){r=release(r,c.id);var existing=complete(c,r),s=readState(c);if(existing){if(!s.current||Number(s.current.build||0)!==r.build){var old=s.current&&Number(s.current.build||0)!==r.build?clone(s.current):s.previous;s.previous=old||null;s.current={version:r.version,build:r.build,packageFile:packageFile(c,r)};s.updatedAt=now();saveState(c,s);}return{ok:true,cached:true,release:r,package:existing,state:s};}
    ...
    if(useBatch){
      try{
        var reqs=[];for(var bi=0;bi<r.modules.length;bi++){var pu=urls(c,r.modules[bi].path,r.ref)[0];reqs.push({url:pu,options:{timeout:Number(c.timeout||8000),headers:{'Cache-Control':'no-cache'}}});}
        var rs=batchFetch(reqs);for(var bj=0;bj<r.modules.length;bj++){var bt=rs&&rs[bj]!=null?String(rs[bj]):'';if(validText(bt)){texts[bj]=bt;sources[bj]=reqs[bj].url;}}
      }catch(be){}
    }
    for(var i=0;i<r.modules.length;i++){
      ...
      var fn=moduleFile(c,r,i),sum=String(md5(text));writeText(fn,text);var reread=String(readFile(fn,0)||'');if(String(md5(reread))!==sum)throw new Error('模块本地 MD5 校验失败: '+String(m.name||m.path));
      files.push({name:String(m.name||('module'+(i+1))),path:String(m.path),file:fn,md5:sum,source:src});
    }
    var pkg={schema:1,id:r.id,version:r.version,build:r.build,ref:r.ref,installedAt:now(),files:files,verify:clone(r.verify||null)};writeText(packageFile(c,r),JSON.stringify(pkg));
  ...
  return{version:VERSION,ensure:ensure,install:install,status:status,complete:complete,readState:readState,removeBuild:removeBuild,stateFile:stateFile,packageFile:packageFile};
})();
