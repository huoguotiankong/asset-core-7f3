...
function fetchOne(u,timeout){var sep=u.indexOf('?')>=0?'&':'?';var t=fetch(u+sep+'_hc_ts='+now(),{timeout:Number(timeout||10000),headers:{'Cache-Control':'no-cache'}});if(!validText(t))throw new Error('无效响应');return String(t);}
...
function latest(c){if(!c.latestPath)throw new Error('配置缺少latestPath');var l=fetchJson(c,c.latestPath,'latest.json');if(!l.id||l.build===undefined||!l.release)throw new Error('latest.json缺少id/build/release');if(String(l.id)!==String(c.id))throw new Error('latest.json应用ID不匹配');var r=normalizeRelease(fetchJson(c,l.release,'release.json'),c.id);if(Number(r.build)!==Number(l.build))throw new Error('latest与release的build不一致');return{latest:l,release:r};}
