...
## 69D. 网盘“下载直链”不得无证据排在专用播放线路之前
PikPak Test22 实机事故：播放详情同时返回通用 `web_content_link` 与 `medias[].link.url`，重构后把前者固定放在第一线路。文件虽然能播放，但会员账号首帧慢、远距离拖动后重新缓冲明显；恢复为官方 `medias` 优先后才回到原应用的播放合同。

固定规则：
- 同一详情同时提供播放专用 media/transcode/origin URL 与下载/原文件 URL 时，先保持官方客户端或已验证样本的线路优先级；不能因为字段名像“原画”就把下载直链抢到第一位。
- 播放与下载输出合同分离：下载 URL 可作为播放器兼容 fallback，但默认线路必须按首帧、Range/seek、CDN、时效与会员能力实测决定。

## 69E. 多线路播放器标记与排序属于输出合同
PikPak Test24 把 `#isVideo=true#` 既附在 `urls[]` 的每条 URL 上，又附在整个 JSON PlayModel 后，且同时改变默认线路排序并移除短时详情缓存；实机出现 0 kb/s、播放异常。截图不足以证明唯一根因，但这种多变量交接变更必须整体回退至 Test23 可播放合同。

- 已实机可播放的多线路模型，要保留 JSON 外层格式、每条 URL 的标记位置、线路顺序及缓存行为；不能只凭 JS 语法/模拟 JSON 解析断定播放器兼容。
- 改画质排序、标记位置或 URL 类型须分别发独立 Test Build，并以首帧、远距离 seek 和临时文件逐项实机确认；失败立即回退，不覆盖旧不可变版本。
- 异步转码/秒传文件若先出现下载直链、稍后才出现专用媒体线路，不得把“已有任意 URL”当成播放准备完成；应有限轮询 media-ready，超时再回退。
- 多线路的 `#isVideo=true#`、Header 等播放器标记应逐线路正确绑定，不能只在整个 JSON 字符串外层模糊追加。
