...
当详情同时返回“专用播放线路”和“下载/原文件直链”时，二者不能仅按清晰度文案混为一类：优先复用官方客户端或已实机验证的 media/transcode/origin 顺序；下载直链通常只作兼容 fallback。验收必须分别测首帧、远距离拖动、Range/分片续播、签名过期与会员线路，不能以“最终能播”代替性能验证。秒传/离线文件也要等 media-ready 后再首选播放，避免过早命中刚生成但 seek 很慢的原文件地址。
...
## 播放列表的媒体类型隔离（2026-09-24）

海阔会依据列表项的 `extra.cls` 组装播放器列表。文件浏览页同时包含目录、视频、图片和普通文件时，只有确认可播放的视频项应设置 `playlist` 类，例如 `playlist video`；文件夹和非视频文件必须使用不含 `playlist` 的类。Magnet/BT 资源页还应先按 MIME 与常见视频扩展名双重过滤，再渲染播放列表，避免“上一个/下一个/列表”混入文件夹或附件。

---

## 动态配置必须先识别“数据形状”和“端点职责”（2026-09-26）

ACFAN Test6 复核发现，官方配置从对象字段变成了“URL 字符串数组”，而旧发现器只遍历对象键，导致整个数组被静默忽略；同时数组中的轮换网站域名并不等于 API Host。

长期 Endpoint Discovery 应先完成两层归一：

```text
Config response
→ 识别 object / array / scalar string
→ 收集所有 URL 候选
→ 按字段语义与真实探针分成 H5 / API / Image / Media
→ 对候选执行对应的低成本业务验证
→ 验证通过后才写 last-good
```
...
- 根数组中的 URL 默认只是未分类候选，不能因为“看起来像域名”直接加入 API Host 列表。
- 不能用 `.work/.site/.cc` 等顶级域名黑名单代替职责识别；轮换业务站常使用这些域名。
