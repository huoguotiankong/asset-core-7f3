...
**统一交付格式（2026-09-24 起）：** 所有需要用户在海阔中覆盖安装或实机测试的小程序版本，最终回复必须直接提供完整、可复制导入的海阔云口令。GitHub 文件链接只作为源码定位或审计补充，不能代替云口令。云口令必须对应当前活动 Stable/Test 的真实 Shell，发布前核对 Shell 数值 version、Bootstrap、Release 和通道元数据一致。
