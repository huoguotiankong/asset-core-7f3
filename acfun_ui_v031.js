// ACFun v0.3.1 clean consolidated UI + official Hiker image decrypt bridge
(function(){
if(typeof ac!=='object')throw new Error('ACFun core missing');
ac.build='2026.08.20-v0.3.1';
ac.imageCipher='xor:2020-zq3-888 / clean FileUtil bridge';

// ---- clean cover extraction -------------------------------------------------
...
ac.__cleanPlainImage=function(u){
    ...
    if(d){if(d.indexOf('//')===0)d='https:'+d;if(!/^https?:\/\//i.test(d))d='https://'+d.replace(/^\/+/, '');return d.replace(/\/+$/,'')+'/'+u.replace(/^\/+/, '');}
    return String(ac.frontendBase||'https://acapp.sexbar.site').replace(/\/+$/,'')+'/'+u.replace(/^\/+/, '');
};

// Exact official Hiker pattern: $(url).image(fn), FileUtil.toBytes(input), FileUtil.toInputStream(bytes).
// System-DNS probe already proved the CDN is reachable, so no extra headers are added here.
ac.__cleanDecryptImage=function(url){
    return $(url).image(function(){
        ...
        with(javaImport){
            ...
            let key=[50,48,50,48,45,122,113,51,45,56,56,56];
...
// ---- clean cards/details: no historical wrappers ---------------------------
...
ac.diag=function(){
    ...
    if(plain){
        ...
        d.push({title:'官方 FileUtil 解密测试',desc:'干净单模块；无 headers、无旧 wrapper。',pic_url:pic,img:pic,url:'hiker://empty',col_type:'movie_3'});
        d.push({title:'验证解密文件头',col_type:'text_1',url:$('hiker://empty#noLoading#').lazyRule(function(url){showLoading('验证中…');try{var r=ac.__cleanVerify(url);hideLoading();refreshPage(false);return 'toast://'+(r.ok?('解密后 '+r.magic):('失败：'+r.error));}catch(e){hideLoading();return 'toast://异常：'+(e.message||e);}},plain)});
    }
    d.push(ac.diagBlock('候选图片字段',getItem('acfun_last_cover_candidates','')||'尚未采集'));
    d.push({title:'复制 0.3.1 诊断',col_type:'text_center_1',url:$('hiker://empty#noLoading#').lazyRule(function(){return 'copy://ACFun '+getItem('acfun_runtime_version','remote')+'\nDecryptMagic='+getItem('acfun_clean_verify_magic','')+'\nDecryptPrefix='+getItem('acfun_clean_verify_prefix','')+'\nDecryptErr='+getItem('acfun_clean_verify_err','');})});
